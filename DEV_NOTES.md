...
- ⛔ **The number is trivial; where it was standing is not.** It sat in `docs/knowledge/a-metric-scoped-to-one-record-ages-silently.md`, a note I promoted in the same session, whose entire thesis is that a figure scoped to something that grows will go stale silently. The note was an instance of itself within two commits.
- ⭐ **`.13.4`'s corpus rule could not have caught it**, and that is the finding worth keeping. Its live-document set is `README.md`, `LIVE_STATUS.md`, `MEMORY.md`, `docs/book/src/*.md`; its exempt set is the dated ledgers, correct as history. `docs/knowledge/` is in neither. The retrievable layer is the one place where a stale number is wrong *every time it is read*, and it was the one place no rule swept.
- ⭐ **The repair I chose is not to update the number.** Bumping 170 to 171 would have reinstated the defect with a fresher value. The argument never needed the total: it rests on `derivations` matching nine records and `resource_references` twelve, which are properties of the KEY and do not grow. So the note stops quoting a size, and says why — naming its own first draft.
- ⭐ **And the mechanism, because a habit is not a mechanism:** `RESTATING_DOCUMENTS` in the write census now lists that note. It restates the census's lineage, so it is part of the census's corpus by construction. ⛔ I deliberately did NOT extend the corpus rule to all of `docs/knowledge/`: a blanket sweep of 42 notes on every correction is the kind of rule people route around, and per-instrument naming is both narrower and enforced.
- 🔎 **The best route of the pass was the compiler.** I had claimed *nothing calls `mcp_listen`* from a `git grep` over the verb names — which cannot see a call through a re-export, a macro, or a name I forgot to grep for. Deleting the module outright and building the workspace answers the same question with no key at all. **When a claim is about reachability, the build system is a better witness than a search.**
- 🔎 **And one route was unkind to me.** Running today's instrument in worktrees at the historical commits showed the residue figures were not *aged* — DOC-0066 already existed when 16 and 14 were published, so they were already behind a record on the day they were written. Correct under their stated definition, misleading under the natural meaning of the word. I had described the failure as slower and more forgivable than it was.
- promotion: declined — the rule is already `a-metric-scoped-to-one-record-ages-silently.md`'s; what this adds is that the note was inside its own blast radius, and that belongs in the note and in `RESTATING_DOCUMENTS`, both of which now carry it.

## 2026-09-19 — The question that priced the decision is the question that caught my error

- To decide whether a malformed dedup window should refuse or repair itself, I had to know what refusing costs. So I censused the callers. The answer was that there are none: outside its own module and its own tests, nothing calls `mcp_listen` at all.
- 🔴 **Which means I had spent three commits describing latent defects as live ones.** *The consequence is a DOUBLE DELIVERY* appears in the lane's census, in two closed leaves, in `LIVE_STATUS.md` and in the changelog. Every one of those sentences is true about the code. None of them is true about the deployment, because nothing reaches the code.
- ⛔ **I did not find this by re-reading my claims.** I found it by needing a number for an unrelated decision. That is the uncomfortable part: the verification pass I would have run on *"is this really a double delivery?"* is the one I did not think to run, because the mechanism was so clearly wrong that the consequence felt settled by it. A mechanism being wrong and a consequence being reachable are two questions and I collapsed them.
- ⭐ **The cheap general form: a defect has a mechanism and a reach, and the reach is a separate measurement.** `CLAIM_VERIFICATION` leg 2 says a population is not a defect count; this is the same rule one step further along — a defect is not an incident until something reaches it. The census that answers it is one `git grep` over the callers, and it costs nothing next to the paragraph it corrects.
- ⭐ **The lane's ordering survived the correction, and for a better reason than I gave it.** I had ranked the transport build last because *new code resting on a dedup window that does not dedup is worse than no new code* — an argument from severity. The caller census turns it into a sequencing fact: that build is the leaf that wires the first caller, so the three repairs land before anything can reach them. I got the order right from a weaker argument than the one that was available.
- ⚠️ **And I repeated a mistake with its own note already written.** The neutralization for this leaf silently did not apply, because `cargo fmt` had reflowed the expression between my writing it and my copying the anchor — exactly what happened at `.6.1.3`, three commits ago, where I recorded it. The assertion caught it both times, which is the argument for the assertion rather than for remembering.
- promotion: pending — *a defect has a mechanism and a reach, and the reach is a separate measurement* is the transferable sentence, with the caller census as its cheapest instrument. Held one more instance before promoting, per the standard I set at `.6.2.1`.
...
- ⭐ **What it actually did is the part that would have been lost.** There was no control observing the two commits separately, so if someone had later "tidied" the gate into the handler's transaction to match `quota.rs`'s general pattern, every test would still have passed and the abuse bound would have quietly become an effect bound. A behaviour nothing asserts is a behaviour the next refactor gets to decide.
- ⛔ **And the reconciliation was real, not cosmetic.** `quota.rs`'s header says the check runs *in the caller's transaction*; this seam runs it in its own. Both statements are true about their own callers, and a reader holding both had no way to know that. Two call sites doing the same-looking thing differently is not automatically a defect — but an undocumented difference is a trap, and writing down which is which was most of the value here.
- 🔎 **An assertion of mine failed and the failure was good news.** I asserted a replayed call returns the original result *byte for byte*; it does not — it carries an extra `"replayed": true`. I had written the weaker, wrong thing while believing it was the stronger one. The truth is better: the same `event_id` proves it is the original EVENT, and the flag proves the pipeline says so rather than leaving a client to infer it.
- ⚠️ **The generalisable bit is that I asserted an equality where I wanted an identity.** `replay == first` would have passed if the system had performed a second contribution that happened to serialise identically. `replay.event_id == first.event_id` cannot. When checking that something is *the same one*, compare the identifier, not the rendering — the rendering can be equal for the wrong reason.
- ⭐ **The control's shape is the other keeper: observe a split by its OUTCOMES.** I could not interrupt between two commits, and any test that tried would have been timing-dependent and flaky. Instead: one caller, two calls, one contribution. The quota moved twice and the event log moved once, which says the two transactions are two transactions without ever touching them.
- promotion: declined — the durable half is the decision record, which is the right layer for a semantic ruling about one surface, and it carries the *difference with a reason* warning aimed at the next reader who would tidy this.
...
- ⭐ **`policy::register` detected a duplicate by letting the INSERT violate the primary key and reading the error.** Outside a transaction that is fine and, arguably, the most honest way to ask: the constraint decides. Inside one it is fatal — PostgreSQL ABORTS the whole transaction on a failed statement, so `authorized`'s audit write, the very next statement, failed with *current transaction is aborted* and the caller got a 500 where an audited refusal belonged.
- 🔎 **`.7.1.2.1` could not have taught me this, and that is the part worth keeping.** `workflows::register` computes `MAX(version) + 1`; it never violates a constraint, so the same move across the same boundary was invisible there. **A template proven twice is evidence about the template, not about the third subject**, and the thing that differed was a detail neither leaf's prose mentions: how each function learns it is being asked for something that already exists.
- ⛔ **The fix is not a savepoint.** `ON CONFLICT (policy_id, version) DO NOTHING` plus `rows_affected() == 0` keeps the unique constraint as the arbiter — no pre-checking SELECT, no reliance on the guard lock for uniqueness — and never raises, so the transaction survives to record why it refused.
- 🔎 **Then the same move exposed a second thing that had been true and harmless for as long as the function existed.** `register` reported EVERY failed INSERT as `Duplicate` and every failed grant lookup as `GhostAuthority`. As a 400 nobody stored, that conflation cost nothing. As an AUDIT RECORD it is a false statement about what happened: a database outage would have written *"that policy version is already registered"* into an operator's trail.
- ⭐ **A refusal that becomes durable may no longer be approximated**, and `authorized`'s own doc comment had already said so in general terms — *a domain refusal is separate from a SQL error, and an unavailable database must never masquerade as one*. The two-level result it prescribes was sitting there; what I had to notice is that turning a response into a record is what makes the rule bite.
- ⚠️ **Both defects were caught by the control, not by review**, and the control caught them because it asserts the refusal's SHAPE — a 403 with a named code and a non-empty audit id — rather than merely that the request did not succeed. An assertion of `!= 200` would have passed on the 500.
- promotion: declined — the site-act template and the *instrument that stops matching its subject* lessons are already `.7.1.2.1`'s, and what is new here is local enough to live at the code: both facts are recorded at `policy::register` and at `site_authority/policies.rs`, where the next person to move a refusal inside a transaction will be standing.
