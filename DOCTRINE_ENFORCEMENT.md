...
| `STORAGE-LOCALITY` | project-owned data lives on the **repository's own volume**, and a name is proved by exclusive creation rather than proposed by a clock. Two patterns, one policy: `env::temp_dir()` — measured on a DIFFERENT volume from the checkout, so data written there leaves the repository's storage accounting — and `subsec_nanos()`/`as_nanos()`, measured at 501 distinct values in 2000 calls with every collision between ADJACENT calls, which is exactly the concurrent case. Paired with `create_dir_all`, which ADOPTS an existing directory, that put two writers in one directory. §13 was prose for the life of the project and was breached in 26 places. Whole-tree; exceptions verbatim in `.doctrine/storage_locality_exceptions.txt`, and a stale entry is also a breach | `scripts/check_storage_locality.sh` (`--self-test`) |
