...
- 🔴 **Every write to `node_inbox` in the server: two quarantine `UPDATE`s, two acknowledge `UPDATE`s, one `DELETE`.** The `DELETE` is the operator prune and its predicate is `acknowledged_at IS NOT NULL` — **delivered rows only** — so an UNDELIVERED row is unreachable by every removal path the server has. `PHASE-3.2.2` deferred *the offline-delivery expiry + max age* to `PHASE-3.5`; `.3.5` closed without them.
