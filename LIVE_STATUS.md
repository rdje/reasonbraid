...
| Phase 1 — trustworthy LAN vertical slice | In Progress | opened by the ADR-002 signature; `.1` complete (`.1.1.1`–`.1.1.3`); **`.1.2` complete** — `.1.2.1` node enrollment, `.1.2.2` authenticated channel + lease/presence, `.1.2.3` inbox retention + quarantine (backlogs 11–14); `.1.3` decomposed into `.1.3.1` invitation lifecycle → `.1.3.2` dispatch-on-accept → `.1.3.3` join/subscriptions; frontier `.1.3.1` |
