...
| Phase 1 — trustworthy LAN vertical slice | In Progress | opened by the ADR-002 signature; `.1` complete (`.1.1.1`–`.1.1.3`); **`.1.2` complete** — `.1.2.1` node enrollment, `.1.2.2` authenticated channel + lease/presence, `.1.2.3` inbox retention + quarantine (backlogs 11–14); **`.1.3` complete** — `.1.3.1` explicit-participants contract (pending invitations, accept/decline/remove, derived expiry, dispatch-on-accept) + `.1.3.2` simple subscriptions (`thread.join`, invite/join-door enforcement); `MAINT-1` done (ephemeral PG test data re-derived to `$ROOT/target/` — §13); **`.1.4` complete** — `.1.4.1` Claude CLI adapter core (the `.4.2` mirror, wire pinned by live probes) + `.1.4.2` env-gated live qualification (first-run pass on the real 2.1.263 harness, money cost, ledger + book + decision record) — backlogs 19–21 done; `.1.5` decomposed — **`.1.5.1` done** (typed §8.5 contribution kinds + evidence references, CLI flags) → frontier `.1.5.2` (rounds) |
