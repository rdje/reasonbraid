...
| Phase 1 — trustworthy LAN vertical slice | In Progress | opened by the ADR-002 signature; `.1` decomposed into `.1.1.1`–`.1.1.3` and **all three done** (aggregate library · identity store · thread command API completion — cancel + typed profiles); frontier `.1.2` (node: enrollment + lease/presence + durable inbox) |
