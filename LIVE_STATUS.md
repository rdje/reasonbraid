...
- 🔴 **NO COMPLETED WORKSPACE RUN EXISTS, and this leaf does not claim one.** `make test` under the pinned runtime reached **78 of ~94 test binaries with ZERO failures** and was then cut off by `ci_browser.py`'s own `COMMAND_SECONDS = 3600` cap (`ci-browser: refused: … timed out after 3600 seconds`, `make: *** [test] Error 2`) — the **first timeout in 16 receipts**, 12 of which read `completed exit=0`. ⛔ Stitching it to `.11.4.7.2.2`'s 94-binary run would be exactly what `knowledge/an-aborted-run-is-not-a-partial-pass` forbids. New owner `.11.4.8`, which also owns the 191 MB re-download the qualified path now costs on every invocation.
...
🔴 **AND THE REASON THE FIRST TWO WERE INVISIBLE IS BLOCKER C1.** `grep -c 'deny\|secret-scan\|gitleaks'` returns **0** for `scripts/check_doctrines.sh`, **0** for `scripts/check_doctrines.project.sh` and **0** for `.githooks/pre-commit`. Both live only in `.github/workflows/supply-chain.yml`, which runs in REMOTE CI — and remote CI has never run. `origin/main` is at 2026-09-12 while the gitleaks finding entered on 2026-09-13, inside the unpushed range, so even a CI run on `origin/main` would not have caught it. ⭐ **So C1 is not only a limit on what may be CLAIMED — it is why two supply-chain gates have been red with nobody able to see it.** That is a different and worse fact than "the authoritative run has not happened yet".
