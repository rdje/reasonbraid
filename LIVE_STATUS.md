...
| Phase 0 — contracts and kill-risk experiments | In Progress | `.0.1`–`.0.8` + `.1.1`–`.1.4` done (WP1 complete); `.2.1` atomic transaction + `.2.2` leased outbox worker done (**WP2 complete**); `.3.1` SQLite node journal + `rb-journal` inspection CLI done (WP3 in progress); frontier `.3.2` outbound node channel with cursor resume |
