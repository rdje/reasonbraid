...
🔴 **B5 IS NOT AN OPEN DECISION — IT IS A DECISION NEVER GIVEN EFFECT (`.13.2`).** Ten manifests declare `license = "MIT OR Apache-2.0"`, and `git ls-files | grep -iE "licen[cs]e|copying"` returns **one path, an unrelated migration**. ⛔ There is no `LICENSE`, `LICENSE-MIT`, `LICENSE-APACHE` or `COPYING` anywhere. A PUBLIC repository whose manifests offer a dual licence while granting nothing, because a manifest is metadata and not a grant — while the project enforces a permissive-only policy on its dependencies. ⚠️ **The only missing input is the copyright holder's name**; which licences, their texts and their location all follow from the ten declarations.
