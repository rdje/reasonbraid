...
| Phase 1 — trustworthy LAN vertical slice | In Progress | opened by the ADR-002 signature; `.1` complete (`.1.1.1`–`.1.1.3`); `.1.2` decomposed into `.1.2.1`–`.1.2.3` — **`.1.2.1` node enrollment done, `.1.2.2` authenticated channel + lease/presence done** (CHANNEL_VERSION 2: key-proof handshake, fencing-token leases, derived presence); frontier `.1.2.3` (inbox retention + quarantine) |
