...
- 🔴 **`migrations/0077` shipped a two-statement backfill that NOTHING drove.** Every PostgreSQL suite applies the migration, so the SQL has always parsed and the upgrade path has always run — but nothing asserted it dates a pre-existing revoked row from its effect record. **A join matching nothing would have been indistinguishable from one matching correctly**, and `migration_upgrade` would have stayed green either way. ⚠️ The leaf opened claiming only that it was UNWITNESSED, not that it was wrong.
- ✅ **The control drives the MIGRATION, not the live write path** — `through(76)`, assert the column is ABSENT (so the fixture cannot be seeding into the post-migration schema and measuring nothing), seed, `through(77)`, read the column back. **Eleven rows, each answer known in advance, every effect record on a DISTINCT past instant**: two dated to their effect's own `effected_at`, nine NULL — no effect record (the pre-`0058` past), `no_op` (a repeated revocation, whose instant belongs to the first), `refused`, an `applied` effect recorded under ANOTHER TENANT, a row that is not revoked, and a wrong-kind effect naming a boundary's id. All eleven landed as the migration header argued, **first run, with no correction to `0077`**.
- ⭐ **WHICH LEG EACH ARM DRIVES IS MEASURED, NOT ASSERTED.** Nine mutations of the migration, each restored byte-identical; **six turn the control RED** — both `UPDATE`s deleted, the grant statement's `outcome = 'applied'` / tenant / `status = 'revoked'` legs dropped, `e.effected_at` replaced by `now()`, and the boundary statement's `operation->>'boundary_id'` leg dropped.
- ⚠️ **THREE LEGS CANNOT BE TURNED RED BY ANY UPGRADE, and that is a property of the schema rather than a hole — recorded instead of left looking covered.** Either statement's `operation->>'kind'` leg is already implied by the id FIELD, because `AdministrativeOperation` has exactly **one** of fourteen variants carrying `grant_id` and exactly one carrying `boundary_id`; it is defense in depth against a future variant that adds one of those names. `g.revoked_at IS NULL` guards a second run the migration ledger does not permit. ⛔ **Fabricating an unproducible row to force those arms red was REFUSED** — it would have measured the fixture's imagination, not the system.
- ✅ **VERIFIED:** `migration_upgrade` **6 passed, 0 failed** (5 at `fd483eb` — measured both sides); **no regression** — `migration_upgrade` + `authority` + `node_work` + `administrative_effects` **65/0**, owned cluster stopped and removed, and `migrations/0077` byte-identical to `fd483eb` after all nine mutations; strict `-D warnings` clippy on the test target rc=0; `cargo fmt --all --check` rc=0; `make book` rc=0; `handoff: OK`. **FALSIFIED** by the six RED mutations — the strongest available form, since the control is red whenever the behavior it names is absent.
...
- 🔴 **`authority/revocation.rs` mutated with `SET status = 'revoked'` and nothing else**, and `migrations/0004` declares both revocation targets with `valid_from`, `expires_at` and `status` — no instant. ⚠️ **NOT a claim the audit trail was insufficient**: `administrative_effects` records every revocation with its actor, reason and `effected_at`. The claim is narrower and structural — the ROW the authorization path already loads cannot date the fact it reports, so `grant_is_live` and the delivery view read something they cannot ask *when* of.
- ✅ **`revoked_at` on `authority_grants` and `enrollment_boundaries`, written by the SAME statement that changes the status.** A second statement would be a second source of truth about one act, and could leave a row revoked with no instant if anything between them failed. ⭐ **The value is `at` — the database time the transaction samples ONCE after the guard wait, which is also what stamps the effect record — so the row and the audit trail agree by CONSTRUCTION rather than by two clocks happening to match**, and the control asserts that equality rather than assuming it.
- ⛔ **THE COLUMN ANSWERS *WHEN* AND NEVER *WHETHER*, which is what makes this safe inside the authority core.** `status` remains the sole answer to whether authority stands, so every liveness predicate is untouched. A NULL on a revoked row means **the instant is not recoverable**, never that the row is live.
- ⭐ **The backfill is DERIVED from the effect record, not guessed** — `{"kind":"grant_revoke","grant_id":…}` joined on the TENANT as well as the id, `applied` outcomes only — and ⛔ **PARTIAL, stated rather than hidden**: `migrations/0058` is where that record begins, so an earlier revocation cannot be dated and those rows stay retained.
- ✅ **`.11.24.1.1.2.1.1`'s deferral CLOSES**: `revoked` inbox rows are prunable on the revocation instant, as a THIRD statement rather than an `OR` — two terminals aged by two clocks cannot report which class was removed by one statement. `deleted_revoked` joins the receipt and the CLI names all three.
- ⭐ **One clause is kept although it is MEASURED redundant**, and the reason is recorded: `AND g.revoked_at IS NOT NULL` changes nothing (`NULL <= $3` is already UNKNOWN — deleting it left all twelve arms green), but **a retention guarantee that depends on the next editor remembering NULL semantics is one clause away from deleting work nobody meant to**.
- ✅ **VERIFIED:** `node_work` **12 passed, 0 failed** — ⭐ the count is UNCHANGED from `41d9f2f` because this leaf REPLACED the `revoked` control rather than adding one, measured at 12 on both sides (the `.13.4.2` habit, now checked by default). `node_inbox` **9/9**, `node_channel` **40/40**, `authority` **22/22**, `authority_transaction` **16/16**, `migration_upgrade` **5/5**, `administrative_effects` **25/25**, `cli_end_to_end` **5/5**. **FALSIFIED in two places**, each restored byte-identical.
- 🔎 **A THIRD TABLE HAS THE SAME SHAPE, found by CENSUSING every writer of a revoked status rather than by recalling one.** `federation_agreements` records `proposed_at` and `accepted_at` and leaves the revocation undated beside them — a sharper instance, because the schema itself establishes the pattern the third column breaks. Owned at `.11.24.1.1.2.1.1.1.1`; ⛔ **not folded in to reach a promotion count of two.**

🔴 **AN EXPIRED ROW PRUNES ON ITS OWN WINDOW, AND A REVOKED ONE HAS NO CLOCK TO PRUNE BY (`.11.24.1.1.2.1.1`, REPAIR-0302).**

⛔ The framing that decided it: **solving *held forever* for DELIVERY and recreating it for STORAGE is not a repair.**

- 🔴 **A row in §10.6's `expired` or `revoked` was never delivered, so it carries no `acknowledged_at` — and the prune deletes on exactly that.** Between `.11.24.1.1.2` giving a queued command a terminal and `.11.24.1.1.2.1` stopping the cursor ack writing a receipt it had not earned, no operator verb could remove one. Those rows are exactly what a broken deployment produces.
- ⭐ **THE RULE: A RETENTION WINDOW MEASURES TIME IN THE STATE BEING RETAINED.** An operator asking to delete rows older than seven days is saying how long they want to SEE a finished row, not when it happened to be created. 🔴 **And the rule does not give the same answer twice, which is the whole decision.** `expired` has an EXACT entry instant — the admitting grant's `expires_at` — so its window measures precisely time-in-terminal. `revoked` has NONE: `revocation.rs` writes `SET status = 'revoked'` and `authority_grants` carries no `revoked_at`. ⚠️ **STALE BY ONE COMMIT and annotated rather than deleted, because it was true when written and is this finding's own evidence (`.13.4.3`): the trigger named at the end of this bullet FIRED — REPAIR-0303 added the column, and `revoked` rows now prune on the revocation instant.** ⛔ `decided_at` EXISTS and would age the row, so the obstacle is not a missing column — it records when the row was CREATED, and a command queued thirty days ago under a grant revoked this morning would vanish under a seven-day window on the day it entered the terminal. Trigger: `.11.24.1.1.2.1.1.1`.
- **THE ORACLES, and where they pushed.** §12.9: *deletion creates a tombstone and reason, not silent disappearance*; §16.11 preserves evidence through quarantine. Both lean towards retention, and they are why the answer is not simply to sweep the terminals — the prune is an explicit AUTHORIZED act with a guard, a reason, an effect record and a measured before/after, and a quarantined row is still never deleted.
- ⚠️ **`deleted` is now the TOTAL, with `deleted_delivered` and `deleted_expired` beside it** — its meaning widened, because it used to be reachable only by delivered rows. ⭐ **The CLI moved with it in the same commit, and was found by CENSUSING the corpus for the receipt's field names rather than by remembering it**: `run_prune_node_inbox` printed *pruned N delivered row(s)*, a sentence this change makes false. `.13.4`'s standing corpus rule applied to a CLI surface rather than a Markdown one.
- 🔴 **THE FIRST FALSIFICATION FOUND A REAL GAP RATHER THAN CONFIRMING THE DESIGN.** Removing `AND g.status = 'active'` left the revoked control GREEN — a revocation does not move `expires_at`, so a merely-revoked grant was excluded by the WINDOW alone and the clause was dead code. ⭐ The control now ages the revoked grant past its expiry too, the case where the two differ: the view calls that row `revoked` (the act outranks the lapse) and **the prune must AGREE WITH THE VIEW** rather than delete it as an expiry.
- ⭐ **The window arm runs BEFORE the deletion arm**, deliberately (`.6.2.1`): a grant that expired two hours ago is NOT removed by a one-day window. Without it a passing deletion would prove only that the row can be destroyed, not that the operator's window governs when.
- ✅ **VERIFIED:** `node_work` **12 passed, 0 failed** (**10** at `71dccb0`, measured both sides); `node_inbox` **9/9**, `node_channel` **40/40**, `quarantine` **1/1**, `mcp_listen` **6/6**, `cli_end_to_end` **5/5**, `administrative_effects` **25/25**. **FALSIFIED, each clause ALONE**, restored byte-identical. ⚠️ **A promotion claim was withdrawn before it shipped**: I wrote that `.7.4.3` was this rule's second instance and checked — that leaf is about WHO supplies the clock, a different mechanism. Recorded, not promoted.

🔴 **A CURSOR ACKNOWLEDGEMENT WAS RECEIPTING ROWS THE TRANSPORT NEVER CARRIED (`.11.24.1.1.2.1`, REPAIR-0301).**

Reproduced as RED through the public surface before a line of the repair was written, which is the order this project requires.

- 🔴 **`acknowledge` marks EVERY row up to the acked cursor** (`WHERE cursor <= $2`) while `replay` withholds rows the node must not receive. The two predicates disagree, so a withheld row gains an `acknowledged_at` the moment the node acks a later row it really did receive — and `migrations/0075` defines that column as *the node process durably holds this command*. Quarantine row 2 of three, let the tail hand the node 1 and 3, ack cursor 3: row 2 carries a receipt. ⭐ The node cannot ack around the hole, so this is the ORDINARY path rather than a contrived ack.
- ⛔ **NOT A COSMETIC COLUMN, and the harm model was already on the record.** `a_fenced_acknowledgement_cannot_mark_another_sessions_delivery` (`.4.2.4`) states it for the same shape: `acknowledged_at` is the retention prune's DELETE predicate, so a false receipt makes work that was never delivered eligible for deletion. 🔴 **The quarantine case is shielded by the prune's own `quarantined_at IS NULL` — and `POST /v1/nodes/replay` clears the quarantine while leaving the false receipt behind, taking the shield with it.** The replayed command then reads `transport_received` without ever having been delivered, and is prunable.
- ✅ **THE REPAIR: one `ACKNOWLEDGE_SQL` both call sites bind**, excluding the rows the tail withheld **for a reason about that row** — a quarantine, and authority that has ended. ⛔ **The two per-NODE reasons stay in deliberately** (no usable certificate, zero declared concurrency): they describe the node's standing now rather than whether a row was carried, and a node can ack during the lease tail, so excluding them would suppress TRUE receipts for rows it demonstrably holds.
- ⭐ **The residual error is the safe one and the design already tolerates it.** Without `offered` (`.11.24.1.1.1`) the server cannot tell *withheld now* from *withheld when offered*, so a row offered before its quarantine and acked after it goes unmarked — re-delivered on the next replay, deduplicated by the node's journal, which is what `migrations/0003` says the channel relies on. **Failing to record a receipt costs a redelivery; recording one that never happened costs the command.**
- 🔎 **THE `.1.2.3` DECISION THIS LEAF OPENED TO ARGUE AGAINST IS A CODE COMMENT ONLY**, measured rather than assumed: `git log -S` dates the sentence to `0e47b27`, `PHASE-1.2.3`'s own commit, in `replay`'s doc comment; that leaf's acceptance says nothing about acknowledgement and no task-tree record carries the claim. ⛔ And the leaf's own opening premise — *the prune depends on it* — was **wrong in the other direction**: the prune excludes quarantined rows outright.
- ⚠️ **The prune's eligibility does NOT move, and the obstacle is not a missing timestamp** (`decided_at` exists): it is that the verb is documented and controlled as deleting DELIVERED rows. Authority-terminal rows are now never prunable — they were prunable before only via the false receipt this removes — so they accumulate. Owned at `.11.24.1.1.2.1.1`.
- ✅ **VERIFIED:** `node_inbox` **9 passed, 0 failed** (**8** at `60affee`, measured both sides); `node_work` **10/10**, its revoked control extended with the column assertion the state assertion could not reach — the CASE ordering keeps that row `revoked` either way, so the state arm alone never touched this predicate. Neighbours unchanged: `node_channel` **40/40**, `quarantine` **1/1**, `node_replacement` **2/2**, `mcp_listen` **6/6**, `node_result_ordering` **6/6**. **FALSIFIED, each clause ALONE**, in situ and restored byte-identical. ⭐ Nothing consumes the narrowed count — `git grep -n "\.acknowledged\b"` over the node and server sources returns no hit.

🔴 **A QUEUED COMMAND FOR A NODE THAT NEVER COMES BACK WAS HELD FOREVER (`.11.24.1.1.2`, REPAIR-0300).**

And that is a property of the code rather than a figure of speech, found by censusing the REMOVAL paths rather than by reading the ladder.

- 🔴 **Every write to `node_inbox` in the server: two quarantine `UPDATE`s, two acknowledge `UPDATE`s, one `DELETE`.** The `DELETE` is the operator prune and its predicate is `acknowledged_at IS NOT NULL` — **delivered rows only** — so an UNDELIVERED row is unreachable by every removal path the server has. ⚠️ **STALE, annotated rather than deleted (`.13.4.3`): true at REPAIR-0300 and superseded by this same session.** Re-derived at HEAD by statement keyword, the census is now **3 `DELETE` · 4 `UPDATE` · 2 `INSERT`** — REPAIR-0302 and REPAIR-0303 added the two DELETEs that reach an undelivered row, which is precisely the repair this bullet's finding set in motion. `PHASE-3.2.2` deferred *the offline-delivery expiry + max age* to `PHASE-3.5`; `.3.5` closed without them.
- ⭐ **§10.6's `expired` and `revoked` are the two ways an undelivered command's AUTHORITY ends — by TIME, and by an ACT.** Both derive from the admitting grant, as the negation of the ONE predicate this repository already uses to ask whether authority stands: `authority::grant_is_live`, the definition `.9.3.1` built by collapsing five spellings. `status <> 'active'` is the act; `expires_at <= now()` is the time. No sixth spelling.
- ⭐ **The link is TOTAL by construction, not sampled** — the premise the whole derivation rests on, so it was proved before anything was written: `selection.rs` returns `Decision::Allowed` ONLY from the branch carrying `grant: Some(grant)`, and a work item is enqueued only on `Allowed`.
- ⛔ **THE MAX AGE IS DERIVED, NEVER CHOSEN, and this leaf picks no number** (`.11.6`). The ceiling is **the admitting grant's own `expires_at`**, a bound the issuing tenant set — §9.2's *retention depends on the operation's retry horizon*. ⭐ The census underneath it: the shipped system has no OTHER delivery-age bound to derive from (the cached-allow TTL recovers by re-ask, the certificate TTL rotates, the lease TTL is presence), so an invented max age would have had nothing to rest on.
- ⛔ **TWO CHEAPER DERIVATIONS WERE REFUSED, and the refusals are the more useful half.** The tenant revocation EPOCH means `CacheVerdict::Stale` — *re-ask* — not a denial, and any revocation in the tenant moves it, so a terminal from it would publish `revoked` for a command whose authority is intact: `.11.24.1.1`'s defect committed again one migration later. Node SUSPENSION is deliberately REVERSIBLE (0017's replacement ritual), and **a terminal derived from a reversible predicate is one a row can LEAVE**.
- 🔴 **THE PRECEDENCE DRAFT WAS REFUTED BY THE CURSOR ACK.** `acknowledge` marks EVERY row up to the acked cursor, withheld rows included — so a terminal below `transport_received` is one the row leaves with nothing delivered. Both terminals sit ABOVE it, and that arm is now a control. ⚠️ Stated rather than discovered later: `consumed` still outranks both, so a command dispatched before its authority was withdrawn reads `revoked` until its result lands.
- ⛔ **The `replay` filter is PART of the repair, not a nicety** — without it a row could reach `revoked` and then be delivered and acked back out of its terminal. ⚠️ **Withheld is NOT deleted**: the row stays inspectable at `GET /v1/nodes/inbox` and the MCP `list_inbox`, so an operator can tell a command that expired while its node was away from one that never existed — `PHASE-3.2.2`'s own offline-KNOWN-versus-unknown distinction, one level down.
- ✅ **VERIFIED:** `node_work` **10 passed, 0 failed**. ⭐ The delta is MEASURED on both sides, not reasoned from the shape of the change (`.13.4.2`'s standing correction): **8** static `#[tokio::test]` at `592994f`, **10** on the working tree. Neighbours unchanged — `node_channel` **40/40**, `node_inbox` **8/8**, `quarantine` **1/1**, `node_replacement` **2/2**, `mcp_listen` **6/6**. **FALSIFIED IN TWO PLACES**, each in situ and restored byte-identical: the derivation (`left: "queued"`, `9 passed; 1 failed`) and the filter (BOTH controls, `left: Some(1)`, `8 passed; 2 failed`). `cargo fmt --all --check` rc=0; `make book` renders; doctrine gate green.
- 🔎 **TWO FINDINGS MEASURED HERE AND OWNED RATHER THAN REPORTED.** `.11.24.1.1.2.1`: **a cursor acknowledgement records a transport receipt for a row the transport never carried** — masked today because `dead_lettered` and now both terminals outrank it, and it must argue against a RECORDED decision (`.1.2.3`). `.11.24.1.1.2.2`: **a dispatched work item's budget reservation expires ten minutes after dispatch and nothing bounds delivery to that window**, so a node polling at minute eleven runs against a ceiling that has re-lent its allowance.
...
- ⚪ **Four §10.6 states stayed underived WITH dispositions and leaves** — ⚠️ **now TWO (`.11.24.1.1.2`, REPAIR-0300, which derived `expired` and `revoked`); the line is corrected rather than deleted because it was true when written and is this finding's own evidence.** Still owed: `offered` and a per-event-type `acknowledged` (`.11.24.1.1.1` — the second is a DESIGN commitment before an implementation).
...
- 🔴 **REPRODUCED AT RUNTIME FIRST, in four legs.** Mallory, enrolled with no site authority, took `red-org-baseline 1.0.0` (**200**); Alice's own registration at that coordinate was refused **400** `already exists`; Alice then read Mallory's clause *"a publication needs no authority"* as the organization baseline, attributed to Mallory's grant; and Mallory appended a `2.0.0` withdrawing the authority clause (**200**).
- ⚠️ **Stated at its real width.** `policy::resolve` names an EXPLICIT `(policy_id, version)`, so this is namespace capture and shared-document extension — **not** the silent re-shaping `.7.1.2.1` measured on the workflow registry.
- ✅ **`policy_register` is a site capability** (`0074`, both CHECK constraints widened; the write, its authorization and its audit commit in one transaction; `grant_is_live` executor-generic so the owning-authority check reads inside it).
- ⛔ **The READS are ASSERTED unchanged** — list, resolve, impact map and the MCP bundle all still answer any enrolled principal, exercised by both tenants after the refusal, so a later repair that bound one fails a control rather than reversing DOC-0071.
- 🔎 **THE CONTROL CAUGHT A DEFECT THE REPAIR INTRODUCED:** detecting a duplicate by letting the INSERT violate the key ABORTS the site transaction, so the audit write failed and the caller got **500** instead of an audited refusal. `ON CONFLICT DO NOTHING` keeps the constraint as arbiter and the transaction alive. 🔎 **And a third:** `register` reported every failed INSERT as `Duplicate` — harmless as a 400, not harmless as an AUDIT RECORD, where an outage would have written a false reason into an operator's trail.
- ⚠️ **Two wire changes, published:** the body now requires a `reason`; and a ghost owning authority and a taken coordinate moved **400 → 403** with an audit id, because each is a question about the database and answering it before the gate is an existence oracle. The five document rules still answer 400 to anyone.
- ⚠️ **The write census follows the repair: `26/17 → 25/16`** — one movement, one cause, the same double step `POST /v1/workflow-profiles` took. `policy_versions` is still site-global and is meant to be.
- ✅ **VERIFIED:** `policy` **21/0** (19 before); `migration_upgrade` 5, `site_authority` 11, `site_registry_http` 8, `mcp_write` 5, `site_operator_cli` 3, `profiles` 62, `mcp` 6 — 0 failed. Falsified in situ with **no test edit at all**; `api.rs` restored byte-identical; 21/21 after. Clippy, fmt, gate (21/21), book + links all rc=0.

🔴 **SEVEN PUBLISHED CLAIMS RE-DERIVED; SIX HOLD, ONE WAS WRONG (`.6.1.5.3.1`, REPAIR-0282).**

Verified against `docs/CLAIM_VERIFICATION.md` — each claim re-derived by a route that does not share a parent with the one that produced it.

- 🔴 **`reads 40 site-global tables → 29` is really `38 → 29`.** Run at `4c5c8e6`, the commit before `migrations/0073`, the census answers **38**. `40` was `.7.1.2`'s figure, stale since `migrations/0072`; it was READ out of `MEMORY.md`, never measured. ⛔ **The arithmetic never closed:** the same bullet published *9 tables left*, and 40 − 9 is 31. 38 − 9 = 29. Corrected in `CHANGELOG.md`, `LIVE_STATUS.md` and the tree.
...
- ⛔ **The LIBRARY stays open by design and the control now ASSERTS it**, so a future repair that bound `GET /v1/policies` fails here instead of silently reversing DOC-0071. ✅ Its WRITE was repaired by `.6.1.5.4` (REPAIR-0283), above.
...
- ✅ **The censuses follow the repair: writes 39/30 → 26/17, reads 38 site-global tables → 29.** 13 rows left and 0 joined; 9 tables left and 0 joined. ⚠️ **SUPERSEDED AS A CURRENT FIGURE by `.6.1.5.4` (REPAIR-0283) — the writes are now 25/16** — and kept verbatim because it was true at its own commit. 🔎 That is also a COVERAGE LOSS — the read census watches tables *without* a tenant dimension, so all nine left it with their 28 readers still unbound. The population is pinned by name in `.6.1.5.3`.
...
- ⚠️ **A POPULATION, not 33 defects.** 🔴 **CORRECTION:** `.7.1.1` published it without checking whether the question was already adjudicated, and it was — `.11.14` is `done` and DOC-0029 decides the site-global data model (evidence content-addressed BY DESIGN, **no table tenant-owned**, the READ tenant-bound). ⭐ **The difference is a number, and it is the residue `.7.1.2` now owns: `.11.14` decided TWELVE tables; a census from the WRITE side finds THIRTY, and 17 are unnamed in the record** — nine of the ten `policy_*`, both `routing_*`, `workflow_profiles`, `agent_profiles`, `profile_versions`, `recruitment_responses`, `quota_events`, `evaluation_trial_results`. ⛔ Cause: `.11.14`'s census was scoped by FAMILY NAME, not derived from the producers.
