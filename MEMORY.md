...
- **Active tree:** `SIGNOFF-REPAIR`; `.2.2.2` closes the disposable test-ownership prerequisite in `REASONBRAID-REPAIR-0004`; next leaf `.3.1`.
- **Next action:** activate `.3.1`, reproduce and fix foreign-tenant grant/boundary revocation before mutation; then `.3.2` explicit site registry authority.
...
- **Defects:** open source-review findings owned by the repair tree; product defect reproductions pending; runner cleanup/spawn races reproduced and fixed. No zero-defect claim.
- **Latest commit:** derive with `git log -1 --oneline`; review baseline `9c2d2ba`.
- **Commit boundary:** `.2.2.2` verified: 40 focused tests, strict Clippy, final restore and three environment controls passed; all owned PG clusters removed. Verify this commit, cleared message file and clean git state before starting `.3.1`.
