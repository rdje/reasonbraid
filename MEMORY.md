...
- **Active tree:** `SIGNOFF-REPAIR`; REPAIR-0051 .11.4.3.1.2.10 fixes CLI removal delegation after clean/census-verified f637955. Real rb reproduces the old 403; an all-TenantWide mutation proves ordinary thread attenuation is guarded. Only removal changes scope. Final five CLI/six server invitation/eleven CLI library tests, strict lint, format, book and source checks pass. Fifteen groups absent; two successful databases removed; two runtime and two preliminary setup failures preserved. Server/evaluator/schema and qualification categories remain unchanged.
- **Next action:** commit REPAIR-0051, clear/verify brief and clean state, consume native census; execute retention fixture clock .11.4.3.1.2.9, then six partial plans .2.7.3 and coverage .2.7.4. Full checkpoint .2, authorized public push/remote CI and CLI transport .3.3.4.3.3.3.3.2.3.2 remain incomplete. Raw: target/cli-removal-controls; preserve stopped run-t4xbqo5e/run-8psce4a0/run-hkq8ugfz/run-8qt01f28 and prior fixture/server/full-b0cddfe/browser/scanner failures and host-startup diagnostics (.11.2). Use the pinned browser wrapper for workspace tests.
