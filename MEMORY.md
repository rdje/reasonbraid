...
- **Active tree:** `PHASE-7` → frontier `.1.1` (**PHASE 5 + 6 CLOSED**; the `.1` block LIFTED — the hardening lane decomposed).. The `.1` lane (the workflow
...
- **Next action:** execute `PHASE-7.1.1` — ADR-034 + the census: the
  hardening contract (the qualified-surface rule, the
  transport-context rule, the defense-in-depth isolation, the
