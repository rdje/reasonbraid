...
- **Active tree:** `SIGNOFF-REPAIR`; child `.2.2.1` complete: supervised focused PG runner, logs/receipts and signal-safe reaping. Next `.2.2.2`.
- **Next action:** execute `.2.2.2` test-side disposable ownership and CI alignment; then `.3.1` tenant-bound revocation and `.3.2` site registry authority.
...
- **In-flight uncommitted work:** none after the `.2.2.1` commit; 12 lifecycle, 4 live PG controls and 9 existing authority tests passed; all results consumed and owned clusters stopped/removed.
