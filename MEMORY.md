...
- **Active tree:** `SIGNOFF-REPAIR`; `.11.4.1` complete in REPAIR-0013: changelog rotation through verified Git history, with no cap increase. Authority repair `.3.3.2` closed in `25ed7d1`.
- **Next action:** activate `.3.3.3` after the clean-tree check: actual-parent and usable-grant selection for caller/delegated/admin-read authority, preserving the frozen-own-tenant boundary-status exception.
...
- **Continuity:** all 95,038 original changelog bytes reconstruct from retained and Git-history segments; new digest 8,462 bytes at this capture. Exact retrieval is in the indexed rotation decision and book introduction. No test/DB job remains. Broader containment remains `.11.4.2`; authority and other concrete repairs remain open.
