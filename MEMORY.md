...
- **Active tree:** `PHASE-6` → frontier `.7.1` (**PHASE 5 CLOSED**; the `.7` lane decomposed — the G3 gate maps over the shipped lanes).. The `.1` lane (the workflow
...
- **Next action:** execute `PHASE-6.7.1` — the G3 evidence census:
  the claim/evidence mapping (the nine Demo-B steps → the shipped
  machinery), the G3 clause map, the §25.1 gate reading. No code; then
  `.7.2` the G3 gate package.
