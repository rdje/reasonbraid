...
- latest_commit: `REASONBRAID-DOC-0047` (leaf `SIGNOFF-REPAIR.3.4.3.1.1.1`): a signed-off leaf reached a right conclusion from a FALSE premise; §4.1 grades the two separately. Re-derive the ahead-of-origin count with `git rev-list --count origin/main..HEAD`; push at ~300.
- active_work_unit: `SIGNOFF-REPAIR` → frontier leaf: `SIGNOFF-REPAIR.7.2.7` (`pending`). ✅ CLOSED this session: `.11.17.2`, `.11.20.1`, `.11.20.2`, `.11.18.1`, `.11.18.2`, `.11.2.5`, `.3.4.3.1.1.1` (+ DOC-0046, the 22nd changelog rotation).
- next_action: `.7.2.7` — 🔴 **the Git acquisition declares a time ceiling nothing reads**: `max_time` in `GitLimits`, defaulted to 120 s, consumed by no code path. ⚠️ R0's identically-named field IS enforced, so tell the two apart before calling either a defect. First product-code leaf after seven doctrine ones. Full statement in the leaf.
