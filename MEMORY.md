...
- latest_commit: `REASONBRAID-REPAIR-0249` (leaf `SIGNOFF-REPAIR.7.2.7`): a refusal wired end to end and constructed by nothing. Re-derive the ahead-of-origin count with `git rev-list --count origin/main..HEAD`; push at ~300.
- active_work_unit: `SIGNOFF-REPAIR` → frontier leaf: `SIGNOFF-REPAIR.7.2.10` (`pending`). ✅ CLOSED this session: `.11.17.2`, `.11.20.1`, `.11.20.2`, `.11.18.1`, `.11.18.2`, `.11.2.5`, `.3.4.3.1.1.1`, `.7.2.7` (+ DOC-0046, the 22nd changelog rotation).
- next_action: `.7.2.10` — 🔴 **`AcquisitionError.kind` is a second refusal vocabulary and nothing documents it.** It is NOT a §9.8 reason code (the struct carries `kind`), so `REASON-CODE-DOC` is right to exclude it. ⛔ The 93 mapped literals in the server are a POPULATION — classify before publishing any of it. Full statement in the leaf.
