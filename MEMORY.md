...
- **Active tree:** `PHASE-6` → frontier `.1` (**PHASE 5 CLOSED**: the G5 gate Met as a subtraction gate — the lift claim withdrawn per §25.1; the `.1`–`.5` lanes + the gate package complete).. The `.1` lane (the workflow
...
- **Next action:** execute `PHASE-6.1` — the semantic policy lane
  (`docs/tasks/PHASE-6.md`, proposed): decompose at the census seams
  first; §25.1's pre-Phase-6 gate applies (no binding policy governance
  unless real owners accept the authority/correction model).
