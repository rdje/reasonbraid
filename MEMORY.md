...
- **Active tree:** `SIGNOFF-REPAIR`; `.3.3.4.3.3.3.1` complete in REPAIR-0026. All 25 selected controls, focused strict lint and book checks pass; every result/shutdown consumed and owned cluster absent. Server bootstrap recovery protocol `.3.3.4.3.3.3.2` is next.
- **Next action:** finish the REPAIR-0026 commit/brief/clean handoff checks, then activate `.3.3.4.3.3.3.2` and refine the selected server protocol before edits. Read docs/decisions/2026-09-09_bootstrap-recovery.md: optional canonical RequestId, server-generated TenantId, immutable full outcome with enrollment, at most one provisional rollback/routing redirect under a shared total budget. Capture keyed feature baseline before implementation. Durable CLI recovery and safe state publication follow in `.3.3.4.3.3.3.3`.
...
- **Continuity:** the no-key bootstrap probe observes COMMIT/PgSleep, HTTP commit uncertainty without IDs, later original committed readback and a second distinct tenant on repeated no-key invocation. Both complete row sets are verified. Nine enrollment + sixteen primitive controls pass; production behavior is unchanged in this contract leaf. The CLI persists IDs only after success and writes state.json in place at its two save paths; its child owns durable pending-state, atomic publication, locality and concurrency qualification. Evidence: docs/tasks/artifacts/signoff_review/bootstrap-recovery.md. No-key requests remain intentionally distinct; keyed server/CLI recovery is not implemented yet.
