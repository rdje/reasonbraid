...
- latest_commit: `REASONBRAID-REPAIR-0223` (leaf `SIGNOFF-REPAIR.11.14.3.4`): the §12.1 reference detail read and the resolve verb are bound to their registrants. Re-derive the ahead-of-origin count with `git rev-list --count origin/main..HEAD`; push at ~300.
- active_work_unit: `SIGNOFF-REPAIR` → frontier leaf: `SIGNOFF-REPAIR.11.14.3.6` (`pending`); `.11.14.3.10` sits beside it at row 1a0.
- next_action: `.11.14.3.6` — `expected_digest` became a reference's IDENTITY and **nothing reads a STORED pin**, so a snapshot of entirely different bytes is accepted against a pinned reference. ⛔ Enforcement is only one of THREE honest dispositions and the leaf must choose with a reason: `evidence_snapshots` is already designed to hold many versions per reference, so "enforce the pin" may be the wrong shape. Measure what reads the column first (13 hits in 3 files, classified).
