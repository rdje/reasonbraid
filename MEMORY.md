...
- **Active tree:** `SIGNOFF-REPAIR`; `.3.3.4.3.3.2` complete in REPAIR-0025. All 97 selected controls, final focused strict lint and book checks pass; every result/shutdown is consumed and all three owned clusters are absent. Bootstrap recovery `.3.3.4.3.3.3` is next.
- **Next action:** finish the REPAIR-0025 commit/brief/clean handoff checks, then activate `.3.3.4.3.3.3`. Reproduce new-bootstrap response loss/commit uncertainty and select a bounded, explicit request identity/outcome contract with durable CLI recovery. Preserve intentionally distinct new tenants and tenant-scoped replay; do not use names as global idempotency. Complete issuance coverage reconciliation `.3.3.4.3.4` follows that prerequisite.
...
- **Continuity:** complete dev enrollment now guards before replay through all writes, uses same-context boundary/grant helpers and database time, preserves validation/replay/default actions/dev issuer behavior, rolls back typed pre-commit errors and reports unconfirmed commit phase. Matched baselines reproduced six failures and exact table deltas; all 97 controls pass after repair. Source audit found no bootstrap request key/outcome lookup: a lost response may leave a client without its generated tenant ID and CLI state. That follow-up is task-owned and explicit in the book, with runtime reproduction pending. Evidence: docs/tasks/artifacts/signoff_review/enrollment-transaction.md. Card import and caller/admission/effect policies remain separately owned.
