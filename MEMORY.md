...
- **Active tree:** `SIGNOFF-REPAIR`; leaf `.2.1` complete: local launcher, verified Cargo cache and Makefile integration. Next `.2.2`.
- **Next action:** execute `.2.2` disposable focused PG verification; then `.3.1` tenant-bound revocation and `.3.2` site registry authority.
...
- **In-flight uncommitted work:** none after the `.2.1` commit; focused core test consumed (49 passed, 1 intentional ignored schema writer); no background job.
