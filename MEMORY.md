...
- **Active tree:** `SIGNOFF-REPAIR`; `.3.2.2` complete in REPAIR-0007: protected operator CLI, 16 focused tests plus 3 strengthened CLI controls, 13 runner controls and strict lint pass.
- **Next action:** activate `.3.2.3` HTTP registry enforcement after the clean-tree check. Follow with `.3.3` actual-boundary authorization, atomic tenant effect auditing and the tracked core subject serde repair.
...
- **Continuity:** CLI verification is consumed; successful clusters stopped/removed, failed clusters verified and removed with evidence in the leaf. No live job to resume. HTTP handlers still use tenant-admin authority until `.3.2.3`. Preserve fresh text operator-role queries. Core subject serde and remaining direct-entrypoint passfile locality have concrete owners `.3.3`/`.11.2`.
