...
- **Active tree:** `PHASE-6` → frontier `.3.1` (**PHASE 5 CLOSED**; the `.2` lane COMPLETE; the `.3` lane decomposed — the compiler is the greenfield).. The `.1` lane (the workflow
...
- **Next action:** execute `PHASE-6.3.1` — ADR-033 + the census: the
  compiler contract (the byte-identical rendering, the target
  vocabulary, the unrepresentable clause as the declared refusal,
  the hermetic crate rule). No code.
