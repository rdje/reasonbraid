...
- **Active tree:** `SIGNOFF-REPAIR`; REPAIR-0052 .11.4.3.1.2.9 repairs the fixed retention-test cutoff after clean/census-verified e515713. Prior current/original fixture failures and database-time predicates prove the calendar assumption. The existing test now derives strict TTL boundaries from actual created_at and checks exact class effects, repeat no-change, audit preservation and replay age. Focused retention and all 31 profiles tests pass, plus strict server lint, format, book/source verification. Seven groups absent; two success databases removed; original failures preserved. Production/schema and progress categories unchanged.
- **Next action:** commit REPAIR-0052, clear/verify brief and clean state, consume native census; execute six partial fixture plans .11.4.3.1.2.7.3 (decompose if needed), then adoption/coverage .2.7.4. Full checkpoint .2, authorized public push/remote CI and CLI transport .3.3.4.3.3.3.3.2.3.2 remain incomplete. Raw: target/retention-clock-controls; preserve prior failed fixtures and host diagnostics. The compiler completed naturally after the idle observation; identity was gone before sampling, so no stack/cause was established (.11.2). Use the pinned browser wrapper for workspace tests.
