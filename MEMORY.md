...
- **Active tree:** `SIGNOFF-REPAIR`; REPAIR-0050 .11.4.3.1.2.8 corrects participant removal's TenantAdmin target to Tenant, preserving the evaluator and locked tenant/thread binding. Two new HTTP controls fail against unchanged production; final six invitation, 22 authority, 33 command-API and ten evaluator controls pass. Strict server lint/book/source checks pass; all thirteen recorded groups absent, three success databases removed, new baseline and prior failures retained. Broader qualification categories remain unchanged.
- **Next action:** commit REPAIR-0050, clear/verify the brief and clean tree, consume native census, then execute CLI delegation companion .11.4.3.1.2.10. It still supplies thread-only scope for administrative removal; actual CLI reproduction is pending. Next are retention fixture clock .2.9, six partial plans .2.7.3 and coverage .2.7.4, then full checkpoint .2, authorized public push/remote CI and CLI transport .3.3.4.3.3.3.3.2.3.2. Raw: target/participant-removal-controls; preserve stopped run-mtrvlow8 and all prior fixture/full-b0cddfe/browser/scanner failures and host-startup diagnostics (.11.2). Use the pinned browser wrapper for workspace tests.
