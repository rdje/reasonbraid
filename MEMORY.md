...
- **Active tree:** `SIGNOFF-REPAIR`; browser test ownership .11.4.3.1.5.1 complete in REPAIR-0038. Eight controls, strict lint and native/source/residue checks pass; all results consumed. Evidence: docs/tasks/artifacts/signoff_review/browser-test-lifetimes.md.
- **Next action:** after clean REPAIR-0038/brief and consumed handoff census, activate .11.4.3.1.5.2 production browser profile/process/handler/network-task lifetime, then combined .5.3, compiler disposition .6 and full checkpoint/push .2. Return to CLI .3.3.4.3.3.3.3.2.3.2 afterward. Preserve target/browser-lifetime-controls, especially case-01a08785-b3c6-71c1-9d01-257f1e9181d8 and failed/final logs. Native zombie-group EPERM is reproduced and the harness now waits boundedly for actual absence; original Chrome member state remains unknown. Final render still requests group stop after worker exit (PID 98352), so production cleanup is not qualified. No full local/remote CI or push has run.
