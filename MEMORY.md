...
- **Active tree:** `PHASE-6` → frontier `.4.3.1` (**PHASE 5 CLOSED**; the `.4.3` half decomposed — the Git write path is the greenfield).. The `.1` lane (the workflow
...
- **Next action:** execute `PHASE-6.4.3.1` — the publication-store
  contract + the census: the decision record (the local bare
  repository, the ref scheme, the staging-branch shape, the write
  path's gix surface — the CAS). No code.
