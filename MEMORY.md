...
- **Active tree:** `PHASE-5` → frontier `.3.1` (**PHASE 4 CLOSED**;
...
- **Next action:** execute `PHASE-5.3.1` — ADR-030 + the
  census: the moderator/synthesizer contract (the moderation
  actions are contributions — the closed vocabulary IS the
  structural prohibition; the appealable action = the
  challenge; the synthesis record = the derived-content
  shape). No code.
