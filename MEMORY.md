...
- latest_commit: `REASONBRAID-REPAIR-0227` (leaf `SIGNOFF-REPAIR.11.14.3.13`): a snapshot names the locator its reference names. Re-derive the ahead-of-origin count with `git rev-list --count origin/main..HEAD`; push at ~300.
- active_work_unit: `SIGNOFF-REPAIR` → frontier leaf: `SIGNOFF-REPAIR.11.14.3.5` (`pending`); `.11.14.3.7` and `.11.14.3.10` also open in this family.
- next_action: `.11.14.3.5` — THREE mechanisms in one goal line, each owed its own reproduction: `POST /v1/resources` takes `scheme` unvalidated while §12.2 ranks resolvers on it (`resources::scheme_of` already exists, so the open question is the compatibility break); `#[serde(default)]` empty strings make `migrations/0023`'s `NOT NULL DEFAULT` clauses dead, and the two writers now disagree (`''` vs `'low'`); and a locator's fragment rides `original_locator`. ⛔ The fragment is the one to leave alone unless decided — splitting it IS canonicalization, which §12.1 defers.
