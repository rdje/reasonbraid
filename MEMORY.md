...
- **Active tree:** `SIGNOFF-REPAIR`; `.3.2` complete: site service, protected operator CLI and all seven registry HTTP operations. HTTP implementation `a710651`, closure REPAIR-0009; 58 selected security tests plus final 12 HTTP/registry controls pass.
- **Next action:** expand `.3.3` into bounded children, starting with the runtime-confirmed core subject serde defect, then actual-boundary authorization, usable-grant selection and atomic tenant effect auditing.
...
- **Continuity:** all `.3.2.3` results are consumed; successful and failed disposable workspaces were verified stopped and removed, with evidence in the leaf. No live verification job remains. Preserve fresh text operator-role queries. Core subject serde, delivery-reader error typing and remaining direct-entrypoint passfile locality have owners `.3.3`/`.5.3`/`.11.2`.
