...
- latest_commit: `REASONBRAID-REPAIR-0224` (leaf `SIGNOFF-REPAIR.11.14.3.6`): a reference's `expected_digest` names its bytes. Re-derive the ahead-of-origin count with `git rev-list --count origin/main..HEAD`; push at ~300.
- active_work_unit: `SIGNOFF-REPAIR` → frontier leaf: `SIGNOFF-REPAIR.11.14.3.12` (`pending`), with `.11.14.3.11` at row 1a and `.11.14.3.10` still open.
- next_action: `.11.14.3.12` — `api.rs`'s R0 and R5 arms call `let _ = crate::snapshots::submit(…)`, so resolving a PINNED reference whose page drifted returns a receipt and no snapshot, silently. Reproduce it with a served-bytes mismatch, RED first. ⛔ Do NOT assume the answer is to fail the resolution: the acquisition DID happen and the receipt is a true statement about the network — the candidates are an `acquisition_error` beside it, a `persisted` field, or failing the request (a wire change).
