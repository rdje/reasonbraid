...
- **Active tree:** `PHASE-6` → frontier `.6` (**PHASE 5 CLOSED**; the `.5` lane COMPLETE — the waves + the receipts + the drift + the corrections; policy 10).. The `.1` lane (the workflow
...
- **Next action:** execute `PHASE-6.6` — the outcome monitoring +
  the scheduled review triggers (§15.11): the elapsed interval, the
  dependency change, the adverse threshold, the external standard
  change, the repeated waiver, the drift, the evaluator regression.
  Decompose at the census seams first (the `.5` lane COMPLETE).
