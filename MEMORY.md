...
- latest_commit: `REASONBRAID-REPAIR-0232` (leaf `SIGNOFF-REPAIR.11.14.1.1`): six suites could not start. Re-derive the ahead-of-origin count with `git rev-list --count origin/main..HEAD`; push at ~300.
...
- standing lesson from 2026-09-17: ⛔ **KNOWING A RULE IS NOT APPLYING IT.** I promoted `a-census-is-as-wide-as-its-key` and then, in the same session, (a) failed to sweep it backwards over an earlier census — `.11.14.3.8`'s was FALSE for seven commits with a live unbound write behind it — and (b) repeated the exact error in `migrations/0067`, sweeping the plans naming `resource_references` and not those naming `tenants`. ⭐ **A sweep is keyed on the NEW THING'S OWN constraints, computed rather than recalled**, and a new rule is run over the instances that came BEFORE it, immediately. Other extensions earned today: a compatibility objection is a claim about the product; a field's name is not its contract — its consumer is; an instrument that reports the same value either way has told you nothing.
