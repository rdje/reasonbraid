...
- **Active tree:** `SIGNOFF-REPAIR`; `.3.3.1` complete in REPAIR-0010: canonical core subject JSON. Core tests pass 49 unit + 3 integration controls; 40 live compatibility controls and core/server strict lint pass. Site repair `.3.2` was closed in `9356379`.
- **Next action:** activate `.3.3.2` bound authority evaluation invariants after the clean-tree check. Subsequent children own actual-parent/usable-grant selection (`.3.3.3`) and tenant authority/effect serialization (`.3.3.4`).
...
- **Continuity:** all `.3.3.1` core/lint/live results are consumed, and owned `run-r7ozj054` stopped/removed. The canonical kind/id parser rejects ambiguous input; the envelope string and database split fields remain compatible. ADR-009's synthetic token-size claim is withdrawn; a real comparison is owned by `.3.4`. Preserve fresh site-operator permission queries. Other concrete repairs remain tracked; no zero-defect claim.
