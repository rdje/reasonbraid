...
- latest_commit: `REASONBRAID-REPAIR-0246` (leaf `SIGNOFF-REPAIR.11.18.1`): `GAP-CLAIM-CENSUS` advised on 69 files and enforced on 15 — the script spelled its population twice. Re-derive the ahead-of-origin count with `git rev-list --count origin/main..HEAD`; push at ~300.
- active_work_unit: `SIGNOFF-REPAIR` → frontier leaf: `SIGNOFF-REPAIR.11.18.2` (`pending`). ✅ CLOSED this session: `.11.17.2`, `.11.20.1`, `.11.20.2`, `.11.18.1`.
- next_action: `.11.18.2` — 🔴 **a calibration's 0 means nothing when the rule already polices the corpus.** Two replays returned 0 of 559 and only one meant anything; the other is survivorship. ⚠️ Neighbour of `calibrate-over-the-history-that-contains-the-instance`, not the same trap. Full statement in the leaf.
