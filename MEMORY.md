...
- **Active tree:** `SIGNOFF-REPAIR`; `.3.3.2` implemented and focused-verified in REPAIR-0011 (verification-pending). Predecessor `.3.3.1` is `e1acb75`.
- **Next action:** consume `run-ufz0yetj` command_api compatibility result and cluster shutdown, resolve any failure, then close `.3.3.2` through a clean commit workflow before `.3.3.3` actual-parent/usable-grant selection.
...
- **Continuity:** `.3.3.2` baseline failures are recorded. Corrected core 51 unit + 3 subject tests and all six server evaluator controls pass; core strict lint passes. Server strict lint and 11 live authority tests also pass. The guarded runner `run-ufz0yetj` continues command_api compatibility confirmation. Consume results and shutdown before closure; no full qualification claim.
