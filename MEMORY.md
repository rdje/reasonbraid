...
- latest_commit: `REASONBRAID-REPAIR-0231` (leaf `SIGNOFF-REPAIR.11.14.3.14`): the acquisition path takes the quota — the director's delegated call. Re-derive the ahead-of-origin count with `git rev-list --count origin/main..HEAD`; push at ~300.
- active_work_unit: `SIGNOFF-REPAIR` → frontier leaf: `SIGNOFF-REPAIR.11.14.1.1` (`pending`) — **TEN suites cannot start**; then `.11.14.3.10`.
- next_action: `.11.14.1.1` — **ten fixture plans name `tenants` without `evidence_citations`**, which FKs it, so the shared checker refuses them before a single test runs: `migration_upgrade`, `bootstrap_recovery`, `cards`, `quota`, `classification`, `mcp_listen`, `federation`, `enrollment_transaction`, `authority_transaction`, `quarantine`. Pre-existing since REPAIR-0213 (attributed by command; reproduced at HEAD with changes stashed). ⛔ Do NOT just add the table to the ten — derive the RULE first: does every plan naming a parent name every child the catalogue reports? If that is computable, the ten are its output.
