...
| `scripts/census_broken_tables.py` | BOTH directions of a Markdown table's boundary: every blank line INSIDE a body with the ROWS it orphans, and every block ABUTTING a table with no blank line, which the table swallows one row per line. `--check` is the `BROKEN-TABLE` gate; `--calibrate N` re-measures it across history. ⭐ Every parsing rule is mdbook's answer, not the specification's: an arity mismatch between header and delimiter is NOT a table, 4 spaces of indent is a code block and 3 is not, and a blank mid-body yields one paragraph rather than a second table. ⛔ It refuses on an unbalanced fence rather than reading every later line as fenced. `--self-test`'s 23 arms include TEN negatives — a blank that legitimately ENDS a table, a table at EOF, a table inside a fence, two adjacent tables separated by a blank, and the list/heading/blockquote that end a table without one — because without them each half of the rule degenerates into "no blank line near a table" or "no prose near a table" | `python3 -B scripts/census_broken_tables.py [--check] [--json] [--calibrate N] [--self-test]` |
| `scripts/census_positional_refs.py` | every positional source reference (`a/path.rs` plus a colon and a line) in tracked Markdown, classified against the tracked tree: `pathed` (the path IS a tracked file), `partial` (a path-suffix of exactly one), `unique` (a bare basename naming one), `dependency` (a `<crate>-<version>` segment `Cargo.lock` pins), `ambiguous`, `unresolved`, `dependency-stale`. `--check` is the `POSITIONAL-REF` gate and refuses the last three; `--at <rev>` pins a census to a commit, which is what lets a leaf tell *the corpus grew* from *the number was wrong*; `--calibrate N [kind ...]` prices any candidate arm across history. ⭐ **Until `SIGNOFF-REPAIR.11.17.2` a reference was `pathed` on the presence of a slash ALONE** — resolvability inferred from the shape of the string while the same instrument consulted the tree for every reference without one. Of **251** such occurrences at `6f91897`, **39** named no tracked file and **one was a suffix of three**, which is the ambiguity the gate exists to refuse. ⛔ A suffix must fall on a SEGMENT boundary: `reasonbraid-core/src/x.rs` ends with the string `core/src/x.rs` and does not contain it as a path, so a plain `endswith` resurrects the founding 93-false-positive bug as a false NEGATIVE — arm 17. ⚠️ `partial` is ACCEPTED and that is a decision, not an omission: it resolves for a reader exactly as the accepted `unique` class does, and refusing it would gate tidiness rather than ambiguity (priced anyway at 4.5% of 200 commits, so it is declined on SHAPE, not on cost). ⛔ It does NOT verify a dependency's in-crate path — that needs the vendored registry, which a cold clone lacks. `--self-test`'s 24 arms include the founding false positive, a version token that must not parse as a position, both directions of every new class, and TWO arms LABELLED as passing before and after the fix | `python3 -B scripts/census_positional_refs.py [--check] [--at <rev>] [--json] [--calibrate N [kind ...]] [--self-test]` |
