...
| `scripts/census_positional_refs.py` | every positional source reference (`a/path.rs` plus a colon and a line) in tracked Markdown, classified `pathed`, `unique`, `ambiguous` or `unresolved`. `--check` is the `POSITIONAL-REF` gate and refuses BOTH ambiguous and unresolved; `--calibrate N` re-prices it across history, incrementally, because a full re-classification per commit does not finish. ⭐ The `unresolved` arm's obstacle is worth knowing: a gate on it would have to tell an illustrative EXAMPLE from a CITATION, which no instrument can do from a basename — so it does not try, and an example may not be written in the positional form instead. ⚠️ **DECLARED LIMIT: a reference containing a slash is `pathed` WITHOUT checking the path exists** — 39 occurrences do not resolve (`SIGNOFF-REPAIR.11.17.2`). `--self-test`'s 12 arms include the founding false positive (a hyphen truncating the path), a version token that must not parse as a position, and both directions of the new refusal | `python3 -B scripts/census_positional_refs.py [--check] [--json] [--calibrate N] [--self-test]` |
