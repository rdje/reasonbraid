...
///
/// ⚠️ THREE classes are removed: rows the node acknowledged holding, rows that
/// were NEVER delivered because the grant that admitted them passed its own
/// expiry (§10.6's `expired`, `.11.24.1.1.2.1.1`), and rows never delivered
/// because that grant was REVOKED (§10.6's `revoked`, `.11.24.1.1.2.1.1.1`).
/// This summary names all three, because *pruned N delivered rows* was a true
/// sentence
...
pub async fn run_prune_node_inbox(
    ...
    Ok(format!(
        "pruned {} row(s) from node {}'s inbox — {} delivered, {} expired undelivered, \
         {} revoked undelivered (before {}, after {}, cutoff {})",
        ...
        response["deleted_revoked"].as_i64().unwrap_or(0),
