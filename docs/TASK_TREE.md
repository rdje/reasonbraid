...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.6.2.1` — **the MCP dedup window retains the OLDEST 64, so the newest deliveries are undedupable**: `push` + `truncate(64)` keeps the FIRST 64, and over 70 deliveries replaying the most recent is ACCEPTED while replaying the oldest is skipped. The consequence is a double delivery. ✅ `.6.2` censused at runtime and SPLIT into `.6.2.1`–`.6.2.4` — three live defects (the window's wrong end, a non-monotonic cursor, a malformed window silently emptied) plus the transport profile, which is a BUILD and ranked last. ⚪ The first-delivery race is recorded as NOT REPRODUCED rather than promoted from a source reading. ✅ `.6.1` is FULLY CLOSED — `.6.1.4` (0286), `.6.1.3` (0285), `.6.1.5` and its seven commits | repo-local |
