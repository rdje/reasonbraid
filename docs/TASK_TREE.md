...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.9.1.2.2` — tranche 3b of the clause reconciliation. `.11.9.1.2` is sized at 5,041 characters and split on the same narrowest-candidate boundary that formed it (1,624 / 1,655 / 1,762), and `.11.9.1.2.1` is closed by REPAIR-0177: 20 clauses, 15 `owned`, 3 `attach`, 1 `handled`, 1 `declined`, 0 `unowned`. 🔴 It found ONE mechanism at THREE sites reached by two records — a lookup keyed on `operation_id`/`command_id` with the owning node never in the predicate, in `event_id_for_operation`, in the receipt's `ON CONFLICT (event_id)`, and in migration 0021's `delivery_state` view — and the cursor high-water mark derived from the table prune deletes from. ⭐ `ATTACH-LANDED`, registered one commit earlier by `.11.11`, fired on this leaf's own three unattached clauses before they were written | repo-local |
