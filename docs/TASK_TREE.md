...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.6.1.1` — all three MCP read tools return data the caller is not entitled to; `policy_bundle` takes its tenant as an UNUSED parameter over a `SELECT` with no `WHERE`. 🔴 `.11.13` is closed by REPAIR-0181, the director's correction: this session SURFACED six findings and reported each as "owned", and the audit says **8 of the 10 named leaves had no acceptance of their own and 6 had no children**. Eight containers are now censused and split into eleven bounded leaves that each carry a reproduce, an owns and an acceptance naming a control to be observed RED first — `.6.1.1`–`.4`, `.9.2.1`, `.9.3.1`–`.3`, `.11.2.3`, `.11.4.2.2`, `.11.12`. ⭐ The gap the ledger could not see: `owned`/`attach` measure whether a clause is VISIBLE to a leaf, never whether that leaf is EXECUTABLE. Rows 1b–1e are `.9.3.1`, `.6.1.2`, `.11.2.3`, `.9.2.1`; tranche 4b is row 2 | repo-local |
