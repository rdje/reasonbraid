...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.14.3.2` — §13.2 step 2: a contribution's `EvidenceRef { uri, digest }` resolves to nothing, though `resource_references` is `UNIQUE (original_locator, expected_digest)` — the same key. ✅ **`.11.14.3.1` IS DONE (REPAIR-0217): the `assess` step records an assessment against the thread.** RED was the error message itself — `unknown field "assessment"`, in a body that carries a payload for `verdict` and one for `synthesis` and none for an assessment. The shipped `evidence_review` profile now records a §12.7 assessment keyed by a claim digest the SERVER computed and membership-checked against that thread, over a snapshot the tenant CITED, with six refusals each named. ⭐ Every gate already existed — the membership check, `.11.14.1`'s citation gate, `.11.14.2`'s authoring binding, `claims::submit`'s excerpt validation — so the repair added a body, a kind and an ordering, and no new rule. No migration. The contribution event and the assessment row commit in ONE transaction; `claims::submit` and `is_cited_by` became executor-generic to allow it. ✅ **`.11.14.3` IS DECIDED (DOC-0037), and the HOLD was wrong** — §13.2 specifies the join and had not been read. ✅ **`.11.14.2` (REPAIR-0215)** bound the assessment reads to the AUTHORING tenant and corrected `.11.14`'s own record. ✅ **`.7.4.3` (REPAIR-0214)** made the retention sweep a site act on the server's clock. ✅ **`.11.14.1` (REPAIR-0213)** bound the evidence reads to the CITING tenant. | repo-local |
