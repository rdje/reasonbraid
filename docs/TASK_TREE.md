...
| [`PHASE-2`](tasks/PHASE-2.md) | `active` | `.1.5.1` — the ADR-008 cached-decision semantics spike (`.1.5` split at the ADR-vs-implementation seam; then `.1.5.2` the node-side cache, `.1.6` incarnations) | repo-local |
