...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.3.4.5` — the ADR-009 envelope measurement; `.3.4.4` censused the tagged codecs and found a live one the leaf never named, where a `join` carrying a decline was accepted as a join | repo-local |
