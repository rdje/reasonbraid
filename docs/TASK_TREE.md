...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.20.2` — 🔴 **A standing lesson that has fired in three consecutive leaves is reachable from nowhere but the tree it was written in.** ⚠️ Decide absorb-versus-new against `a-scoping-defect-errs-in-one-direction`, which already carries half of it; the census confirms the pointer resolves by reporting the warning `anchored:method`. ✅ **`.11.20.1` IS DONE (REPAIR-0244): the memory census saw 2 warnings where 8 stood, and 47% of the file by weight was invisible to it** — its model was one bullet where the template it was conformed to uses several. ⭐ The new model is parsed from `MEMORY_ARCHITECTURE.md` §6, never from the measured file. Weight now travels with every row. ⛔ **Two falsifications were needed** — the negative arms have no red against the old model, so a DEGENERATE rule was run to fire them, and that exercise exposed an arm raising `IndexError`, a RED naming nothing. ✅ Re-deciding `UNCITED` changed its answer: 1 of 4 was in NO durable layer, an environment fact written into layer A this same session, now a decision record. ✅ `.11.17.2` (0243), `.11.20` (0242), `.11.17.1` (0241). | repo-local |
