...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.14.3.4` — registering a citation puts its locator in `resource_references`, which any enrolled principal reads by `resource_id`, and `.11.14`'s per-table verdict for that table answered the COLUMN question rather than the read one. ⚠️ An ORACLE, not an enumeration. ⛔ Do NOT assume the answer is a citation table — a reference is a locator a contributor CHOSE to cite, weaker than an acquisition receipt, and a per-tenant set would break the dedupe the pair key exists for. ✅ **`.11.14.3.8` IS DONE (REPAIR-0222): `POST /v1/assessments` is bound to the citing tenant.** 🔴 The oracle was THREE answers, not two — `the cited snapshot does not exist`, `the excerpt does not appear in the snapshot's bytes`, and a **`200`** saying a chosen substring appears in bytes the caller was never allowed to read, with a row written. A census of every surface naming a `snapshot_id` found **seven, six citation-bound, exactly one not**. ⭐ **The compatibility objection was refuted by measurement rather than accepted**: every READ of that snapshot already answers a non-citing tenant `404`, so the cross-team workflow it protected cannot function — the excerpt check ran over bytes the caller can neither see, list nor delete. The gate lives in `claims::submit`, the function both writers reach; the `assess` step keeps its own check as the NAMED refusal while the store's is the invariant. ⚠️ Two principals inside ONE tenant still alias on that route (`.7.4`). ✅ **`.11.14.3.3`** (REPAIR-0219); ✅ **`.11.14.3.2`** (REPAIR-0218); ✅ **`.11.14.3.1`** (REPAIR-0217); ✅ **`.11.14.3`** (DOC-0037). | repo-local |
