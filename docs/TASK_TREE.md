...
| [`PHASE-7`](tasks/PHASE-7.md) | `active` | `.2.4` — the public-enrollment contract (the `.1` lane COMPLETE; `.2` done: the census → decomposed; `.2.1` ADR-027; `.2.2` SECURITY.md; `.2.3` the signed release manifests) | repo-local |
