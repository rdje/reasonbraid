...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.9.1.1.3` — tranche 2c of the clause reconciliation, the last four records of tranche 2. `.1.1.1` reconciled six records (27 clauses) and `.1.1.2` four more (19), the ledger's first `declined` row among them; 15 `attach` clauses are now written into `.4.1`, `.5.1`, `.7.1`, `.8.1`, `.9.2`, `.10.2` and `.11.4`. `.11.10` is closed by REPAIR-0172: `regions::route` reported any `sqlx::Error` as an undeclared-region verdict, and now returns a typed storage failure. `.4.1.6` is closed by REPAIR-0174: an unvalidated host claim panicked the enrolment handler and left its node id unenrollable until the token lapsed; the claim is now checked at issuance, against the certificate library's own verdict. Row 1b is `.11.11`, the `attach`-landing gate the director's grading question produced | repo-local |
