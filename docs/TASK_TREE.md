...
| [`PHASE-7`](tasks/PHASE-7.md) | `active` | `.1.4.1` — the declared-profile contract (`.1.1` done: ADR-034; `.1.2` done: the mTLS identity; the `.1.3` lane COMPLETE; `.1.4` done: the census → decomposed `.1.4.1` contract → `.1.4.2` the secret-store profiles → `.1.4.3` the classification controls) | repo-local |
