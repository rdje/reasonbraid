...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.6.2.4` — the MCP reauthorization/reconnect/transport profile has no wire evidence, and it is the leaf that would WIRE THE FIRST CALLER onto `mcp_listen`. ⚠️ A BUILD, not a repair: its acceptance must quote the documented profile from its source BEFORE implementation. ✅ `.13.4.1` CLOSED (REPAIR-0291) — the director's *ensure the findings hold*: **sixteen published figures re-derived by independent routes, fifteen hold**; the COMPILER settled *nothing calls `mcp_listen`* by deleting the module (0 errors), and today's instrument re-derived 30 · 29 · 27 from historical SOURCE. 🔴 The one that failed was mine and went stale inside the session — a corpus size standing in `docs/knowledge/`, which the corpus rule does not reach. ✅ `.6.2.1`–`.6.2.3` (0288–0290); ✅ `.6.1` FULLY CLOSED | repo-local |
