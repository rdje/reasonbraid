...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.4.1.2` — redemption does not re-check the issuer's authority, a bearer-semantics DECISION rather than a presumed defect; `.4.1.3`/`.4.1.3.1` closed revocation's two halves (a running node renewed its own lease for ever, and was still handed new work inside the tail) | repo-local |
