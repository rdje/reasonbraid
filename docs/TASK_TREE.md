...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.3.5.5` — **`GET /v1/nodes/presence` asks nobody who is calling**: no principal, no authorization and no tenant predicate, on a `node_presence` view that carries `tenant_id` — while the same view is gated at `/v1/admin/nodes/presence`. Source-measured; reproduce before repairing. ✅ **`.3.5.4` IS DONE (REPAIR-0260): the read census could not see 34 of its own routes, and its population had no producer** — `.3.5.3` published `24 get(…) routes in api.rs` when its own commit had **54**, and `rb-server.rs` merges three routers for **58**. `scripts/census_get_route_binding.py` now derives it; all 58 classified **24 authorize · 6 derive-and-bind · 24 site-global BY SCHEMA · 4 unauthenticated**, and the 24 site-global ones are the false-positive class a column-grep would have reported as defects. ⭐ **The instrument found five defects in itself before one in the product**, including a false negative reading 11 gated admin routes as ungated. 🔎 Also opened: `.11.2.6` — the acceptance gate validates **1** of this tree's **194** `ROOT CAUSE` checklists. | repo-local |
