...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.4.7.3` — **blocker C2's owed work**, the only register row that is work here now: four gate records restate a “shipped” count nothing re-derives, and this leaf owns G3's seven verdict claims. ✅ **`.4.2.3.1` (REPAIR-0263)**: the lease was written by the server process's clock and read by the database's — driven ten minutes ahead, the unrepaired product granted **660.0 s** of lease against a published **60 s**. The database clock now owns the column at all five sites; three of them are unfalsifiable on a one-host fixture and are labelled, not covered. The census found a **third** writer where `.4.2.3` recorded two. ✅ **`.11.2.6` (REPAIR-0262)**: the acceptance gate read **1** of this tree's **194** checklists and hard-gated a vocabulary the project does not use — both halves from one cause, an inert control. Scope is now the deepest leaf the commit closes; families are a census-derived `.doctrine/` seam; calibrated 57.7% → **20.6%**; `--debt` counts **73 of 281** closed leaves rather than backfilling them. ✅ **`.3.5.5` (REPAIR-0261)**: `GET /v1/nodes/presence` asked nobody who was calling. | repo-local |
