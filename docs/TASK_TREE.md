...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.14.3.5` — `POST /v1/resources` takes `scheme` as an unvalidated caller field while §12.2 ranks resolvers on it; its `#[serde(default)]` empty strings make `migrations/0023`'s `NOT NULL DEFAULT` clauses dead; and a locator's fragment rides `original_locator`. ⭐ The scheme check already exists (`resources::scheme_of`) — what is undecided is the compatibility break, not the mechanism. ⚠️ THREE mechanisms in one goal line, each owed its own reproduction. ✅ **`.11.14.3.13` IS DONE (REPAIR-0227): a snapshot names the locator its reference names.** ⭐ The census shaped it — NOTHING reads `evidence_snapshots.original_locator` to make a decision, so it is an evidence-integrity defect and a refusal is the whole repair. Byte equality on purpose: the leaf's *"an equality check is a canonicalization decision wearing a different name"* is true of a NORMALISING check and false of a strict one. `final_locator` stays free and the control asserts it. ⭐ The check found a defect in this session's own test data on its first run. ✅ **`.11.14.3.11`** (REPAIR-0226); ✅ **`.11.14.3.12`** (REPAIR-0225); ✅ **`.11.14.3.6`** (REPAIR-0224); ✅ **`.11.14.3.4`** (REPAIR-0223); ✅ **`.11.14.3.8`** (REPAIR-0222). | repo-local |
