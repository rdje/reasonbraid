...
| [`PHASE-7`](tasks/PHASE-7.md) | `active` | `.1.3.2` — the quotas (`.1.1` done: ADR-034; `.1.2` done: the mTLS identity; `.1.3` done: the census → decomposed; `.1.3.1` done: the RLS defense-in-depth — the fail-closed command-core policies + the measured probe-role refusal) | repo-local |
