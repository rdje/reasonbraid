...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.24.1.1.2.1` — **a cursor acknowledgement records a transport receipt for a row the transport never carried**: the ack marks every row up to the cursor, withheld rows included, while `migrations/0075` defines that column as *the node process durably holds this command*. It must argue against a RECORDED decision (`.1.2.3`), and the prune's eligibility depends on the column it would stop writing. ✅ **`.11.24.1.1.2` (REPAIR-0300): a command cannot outlive the authority that admitted it.** A queued command for a node that never came back was held FOREVER — the only `DELETE` against `node_inbox` is the prune, predicated on `acknowledged_at IS NOT NULL`. §10.6's `expired`/`revoked` are the two ways an undelivered command's AUTHORITY ends (time, act), derived from the admitting grant; ⛔ **the max age is DERIVED, never chosen — the grant's own `expires_at`**. Two cheaper derivations refused; the precedence draft was refuted by the cursor ack. 🔴 **`.11.24.1.1` (0297) opened on a MISSING state and found a WRONG one:** `delivery_state` published `acknowledged` for §10.6's `transport_received`, the one state the suite never asserted; `migrations/0075` renames it. 🔴 `.11.24.1` (0296) adjudicated 18 deferrals and 12 goal items and found **seven stranded**, five unknown before. ✅ `.11.24` (0295) shipped the census and the `LEAF-ID-UNIQUE` gate; ✅ `.6.5` (0294): nothing serves the MCP tools (`.6.8`) | repo-local |
