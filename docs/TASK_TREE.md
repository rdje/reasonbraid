...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.13.4.3.1` — **`migrations/0077`'s backfill is shipped and no control drives it**: every pg suite applies the migration so the SQL is valid, but nothing asserts it dates a pre-existing revoked row from its effect record. ⛔ `.7.1.2.2.1`'s shape applied to a DATA MIGRATION. ✅ **`.13.4.3` (REPAIR-0305): 22 claims re-derived over `0300`…`0304`, 20 hold** — 🔴 both failures one mechanism, *a measurement narrowed by something the sentence did not carry*; both conclusions survive on corrected evidence; ⭐ **no new note, because the layer already named the habit and its author did not consult it.** ✅ `.11.24.1.1.2.1.1.1.1` (0304): no column — the pattern it would complete is one nobody reads. ✅ `.11.24.1.1.2.1.1.1` (0303): revoking an authority records when. ✅ `.11.24.1.1.2.1.1` (0302): each terminal prunes on its own clock. ✅ `.11.24.1.1.2.1` (0301): the cursor ack stops receipting rows the tail withheld. ✅ `.11.24.1.1.2` (0300): a command cannot outlive the authority that admitted it | repo-local |
