...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.6.1.5.1` — **the approval that discards the tenant its siblings keep**: `record_policy_approval` tests `reader_tenant(…).is_some()` and throws it away, while the proposal and decision verbs pass theirs into an `rls::with_tenant_claim` check — and `publications::stage` reads approvals to decide whether a publication may be staged. ✅ `.6.1.5` CLOSED (DOC-0071): **the question had one name and two subjects** — the policy LIBRARY is site-wide by design, the nine LIFECYCLE tables are tenant-owned by omission, because the lifecycle already derives and RLS-enforces the tenant at write time and then stores none of it. **47 SQL sites, not the six it was opened over.** Split into `.1`–`.4` | repo-local |
