...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.6.1.5.4` — the policy LIBRARY takes site-operator authority, the last open child of `.6.1.5`: `POST /v1/policies` admits any enrolled principal over a first-come `(policy_id, version)` namespace. ⛔ `GET /v1/policies` and the resolve verb STAY on enrolment, and `.6.1.5.3`'s control now asserts that. ✅ `.6.1.5.3` CLOSED (REPAIR-0281): ten lifecycle list reads bound, both ways round over ten tables — and two predicates had no control until falsification said so. ✅ `.6.1.5.2.1` CLOSED (REPAIR-0280); ✅ `.7.1.2.2.2` (0279); ✅ `.6.1.5.2` (0278); ✅ `.7.1.2.2.1` (0277) | repo-local |
