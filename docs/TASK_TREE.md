...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.17.1` — **9 positional references name a file that does not exist**, 8 of them `init.rs` in this tree and one the `POSITIONAL-REF` registry row's own illustrative placeholder. 🔎 That placeholder is the leaf's real content: gating this class means telling an EXAMPLE from a CITATION, which the ambiguity gate never has to do. ⛔ `.11.6` binds — whether `init.rs` was renamed, deleted or never tracked is unmeasured and the disposition depends on which. ✅ **`.11.19.2` IS DONE (REPAIR-0240): the MIRROR of `.11.19` — a table SWALLOWS the block that abuts it with no blank line.** `PHASE-3`'s eleven-line *"Tree complete"* paragraph was rendering as eleven table rows padded to four columns, under a header that had no rows of its own; repaired to the convention `PHASE-1` and `PHASE-2` already use (prose, no table). Renderer: data rows **59 → 48**, tables **3 → 2**, the paragraph now a `<p>`. ⭐ **`BROKEN-TABLE` gains it as a SECOND ARM, calibrated over the FULL 551-commit history** rather than a window — because the `PHASE-3` instance is older than 200 commits, so the usual window could not see the arm being added at all: **19 commits blocked (3.4%), and every one of the nineteen introduced a defect this work has since repaired.** ⛔ Not "3.4% looks acceptable" — the blocked set and the defect set are the SAME set. ⚠️ The first key over-counted the population by a third (15 against 11): a bullet list ENDS a table, plain prose does not, and that is not uniform across constructs. ✅ `.11.19.1` (REPAIR-0239), `.11.19` (REPAIR-0238), `.11.16` (REPAIR-0237). | repo-local |
