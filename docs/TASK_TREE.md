...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.3.4.1` — delegability at evaluation: the evaluator never reads `grant.delegable`, so a non-delegable grant backs a delegated request; `.3.4` is censused and split into five children | repo-local |
