...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.9` — a census record routed to a leaf is not reconciled against that leaf's split; **`.4.2`'s seven children are ALL closed** (rotation ordering, proof replay, the proof ladder, both check-vs-commit windows, the panicking hex decoder, the secret-store claim) | repo-local |
