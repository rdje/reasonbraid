...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | 🔴 `.11.14.1.1` — **TEN fixture plans cannot start.** `migrations/0062` added `evidence_citations` with a foreign key to `tenants` and did not sweep the plans that name `tenants`, so the shared checker refuses them before a single test runs. Pre-existing since REPAIR-0213 — attributed by command and reproduced at committed HEAD with this session's changes stashed. ⛔ Do NOT just add the table to the ten: derive the RULE first. ⚠️ The full-checkpoint record's *"40 of 40 database suites, 291 tests"* predates the table. ✅ **`.11.14.3.14` IS DONE (REPAIR-0231) — the director's delegated call.** §16.11's `resolver` and `destination` scopes shipped in the vocabulary and were written by NOTHING, while the acquisition path carried no quota, no storm control and no breaker. 🔴 **The finding is WHY**: fail-closed works for the two bound scopes because the server CREATES their members and seeds them; the resolver space grows at runtime and the destination space is the open internet — **a fail-closed bound over a space you cannot enumerate is an outage**. The contract is kept and the mechanism changes: a per-tenant DEFAULT row at `*`, a specific row overriding it, and the absence of BOTH still the typed refusal. ⛔ The ceilings are dev-profile defaults — what was decided is the SHAPE of the bound, not its number. ✅ **`.11.14.3.15`** (REPAIR-0230, the census correction); ✅ **`.13.5`** (DOC-0043, the register column); ✅ **`.11.14.3.7`** (REPAIR-0229); ✅ **`.11.14.3.5`** (REPAIR-0228); ✅ **`.11.14.3.13`** (REPAIR-0227); ✅ **`.11.14.3.11`** (REPAIR-0226). | repo-local |
