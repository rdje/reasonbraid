...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.3.4.1.1` — make the delegate-without-scope pair unconstructible; `.3.4.3` ran the cache's freshness window from the earlier of the server and node clocks, and opened `.3.4.3.1` for the opposite skew direction | repo-local |
