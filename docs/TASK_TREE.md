...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.7.2.7` — 🔴 **The Git acquisition declares a time ceiling that nothing reads**: `max_time` in `GitLimits`, defaulted to 120 s and consumed by no code path, found by `.7.2`'s mechanism table and never discharged. ⚠️ R0's identically-named field IS enforced, so the two must be told apart first. ✅ **`.3.4.3.1.1.1` IS DONE (DOC-0047): a signed-off leaf reached a right conclusion from a FALSE premise, and §4.1 grades the two separately** — *nothing enforces the stored value* against three production sites. ⭐ Read from the pre-repair source at `9446034`: `rotate` sampled `now` after signing, `enroll` 11 awaits and 16 database calls before it. The refusal window is real and UNREACHABLE — the final sub-second of a 600 s certificate against a rotate trigger at the halfway point. ⇒ prose holds, re-derived; named instance withdrawn. ✅ `.11.2.5` (0248), `.11.18.2` (0247), `.11.18.1` (0246). | repo-local |
