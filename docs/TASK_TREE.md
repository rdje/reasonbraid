...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.4.1.4` — the token-issuance route says "an authorized human" and accepts an agent role (`.11.9`'s first routing output); `.4.2`'s seven children are ALL closed, and `.11.9`/`.11.4.2.1` delivered two tracked census instruments and rejected both gates they proposed | repo-local |
