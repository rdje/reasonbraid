...
| [`PHASE-2`](tasks/PHASE-2.md) | `active` | `.1.2.1` — cert issuance at enrollment (`.1.2` decomposed at the issuance-vs-channel seam; `.1.1` done: ADR-006/007 accepted — the spike-measured project-local CA model; then `.1.2.2` channel v3 + rotation → `.1.3` revocation → `.1.4` delegation → `.1.5` caching → `.1.6` incarnations) | repo-local |
