...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.11` — the `attach`-landing gate, now the frontier's first row because tranche 2c took that population from 15 rows to **26** across nine owner leaves. `.11.9.1.1.3` is closed by REPAIR-0175: tranche 2c classified **38 clauses** (21 `owned`, 11 `attach`, 6 `handled`, 0 `unowned`) and completed tranche 2 at 14 records and 84 clauses. It opened no new leaf — `.11.2` and `.11.3` already owned every surface — and wrote all 11 `attach` clauses into `.5.2`, `.11.2` and `.11.3` in the same commit. 🔴 Its heaviest finding is `check_table_arity.sh`: asked of mdbook's own renderer rather than of the GFM spec, a pipe inside a code span DOES split a cell, so the gate's model is wrong and its self-test encodes the error — 2 tracked rows lose content today while the gate reports 0. Row 1b is `.11.9.1.2`, tranche 3 | repo-local |
