...
| [`PHASE-1`](tasks/PHASE-1.md) | `active` | `.1.8` — the G1–G2 exit + Demonstration A (`.1.7` complete: `.1.7.1` `make dev` + `.1.7.2` release packaging + LAN runbook; `.1.6` complete: budget read + embedded shell + demo beat; `MAINT-1`…`MAINT-3` closed) | repo-local |
