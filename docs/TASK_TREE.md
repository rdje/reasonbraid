...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.7.3.6` — **the only un-discharged strand of any of the five gate records**: the R3 pack advertises policy lines it does not enforce (`.7.3.5` found two, **18** advertisements exist, the rest unchecked). ✅ **BLOCKER C2 IS CLOSED (`.11.4.7`, REPAIR-0265)** — all five gate records re-derived line by line, each verdict carrying its command. ⛔ No record's conclusion changed: G6–G7 stays NOT MET for Internet exposure, G3 stays blocked as binding use. ✅ **`.11.4.7.4` (REPAIR-0265)**: G4 and G5 — 3 stand, G4 must be re-earned; **4 of 6 evidence pointers moved** (`profiles.rs` 23→56, and `profiles 23` is G4's only citation). ✅ **`.11.4.7.3` (REPAIR-0264)**: G3 — 3 stand · 1 narrows · 3 re-earned. ✅ **`.4.2.3.1` (REPAIR-0263)**: the lease was process-written and database-read — **660.0 s** granted against a published **60 s**; the database clock now owns the column. | repo-local |
