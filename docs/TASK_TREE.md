...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.24.1.1.2` — §10.6's `expired` and `revoked` terminals have no producer, so **a queued command for a node that never comes back is held forever**; this absorbs `PHASE-3.2.2`'s deferred offline-delivery expiry (0 hits in `crates`). 🔴 **`.11.24.1.1` (REPAIR-0297) opened on a MISSING state and found a WRONG one:** the derived `delivery_state` published `acknowledged` for what §10.6 calls `transport_received`, in a field `GET /v1/nodes/inbox` and the MCP `list_inbox` both return — and it was the one state the suite never asserted. `migrations/0075` renames it. 🔴 `.11.24.1` (0296) adjudicated 18 deferrals and 12 goal items and found **seven stranded**, five unknown before. ✅ `.11.24` (0295) shipped the census and the `LEAF-ID-UNIQUE` gate; ✅ `.6.5` (0294): nothing serves the MCP tools (`.6.8`); ✅ `.6.2` FULLY CLOSED with `.6.2.4` (0293) | repo-local |
