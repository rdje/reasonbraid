...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.14.3.13` — a snapshot records an `original_locator` its own reference need not carry; `snapshots::submit` binds the submission's value straight into the insert and never compares it. ⛔ Do NOT assume the answer is an equality check: §12.1 keeps canonicalization separate and scheme-specific, so an equality check is a canonicalization decision wearing a different name. ✅ **`.11.14.3.11` IS DONE (REPAIR-0226): a snapshot is filed against a reference its own tenant registered.** 🔴 RED was not merely an oracle — the foreign submission **succeeded**, attaching a snapshot to another tenant's reference, and `grep -rn "WHERE reference_id"` returns **1** site so nothing can list it. ⭐ It was missed by `.11.14.3.4`'s own census one commit earlier, which enumerated the routes under `/v1/resources` while this route names a reference in its BODY — not wrong, **silent**. Rule promoted: enumerate on the IDENTIFIER, not the address shape (second instance after `.3.5.3`). The binding is a predicate in the same statement with **no new error**, so a foreign reference answers the `ReferenceMissing` an absent id gets. ✅ **`.11.14.3.12`** (REPAIR-0225); ✅ **`.11.14.3.6`** (REPAIR-0224); ✅ **`.11.14.3.4`** (REPAIR-0223); ✅ **`.11.14.3.8`** (REPAIR-0222). | repo-local |
