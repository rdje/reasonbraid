...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.3.3.4.11.1` — the transactional profile writer: the `_in_tx` body and the role/version anchor, the prerequisite `.11.2` guarded attestation and `.11.3` atomic card import both need | repo-local |
