...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.6.1.5` — **the policy registry, re-scoped by DOC-0066 from one table to TEN**, which are ONE control chain: `publications::stage` checks parentage and never ownership, and `reviews::schedule_reviews` reads drift, outcomes and corrections with NO predicate and schedules reviews against other tenants' publications. ⭐ `.7.1.2.1` settled the WORKFLOW registry the same way — site-operator authority, chosen because the code's own comment already claimed it — and is the precedent to follow or differ from by name. ✅ `.7.1.2.2` CLOSED (REPAIR-0274): both routing journals bound to their own tenant; the leak had carried the other tenant's PRINCIPAL. The census follows the repairs, 42/33 → **39/30**, residue 17 → **14** | repo-local |
