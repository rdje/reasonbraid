...
| [`PHASE-8`](tasks/PHASE-8.md) | `active` | `.4.4` — the signed plugin registry/allowlist (opened at the Phase-7 close; **the `.1`/`.2`/`.3` lanes COMPLETE**; `.4` the adapter/resolver SDK census → decomposed `.4.1`–`.4.4`; `.4.1` the SDK contract; `.4.2` the compatibility matrix; `.4.3` the certification suite) | repo-local |
