...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.3.5.4` — the read census `.3.5.3` could not finish: 10 of 24 GET handlers delegate their SQL to a module, so the per-handler scan that found the inbox leak cannot see them. ✅ **`.11.2.4` IS DONE (REPAIR-0258): a gate blind to the abbreviation of the very noun it is built on** — all four clauses anchored on the full word, so two live instances sat in the corpus invisibly; the widening was derived from a ~60-line census (delta 3 → 11, none lost), and **six of the eight new matches were the project documenting its own blind spot**. ✅ **`.11.22` (REPAIR-0257)**: the frontier table gained a row per state change and never retired the old one — `FRONTIER-STATUS` registered at 0.06 s, and it caught this very leaf leaving a stale row behind. ✅ **`.11.21` (0256)** `RUST-FORMATTING`; **`.11.8.1` (0255)** the route key too loose and too tight at once; **`.7.4.4` (0254)** citation withdrawal vs shared-row tombstone; **`.7.3.2.1` (0253)** 1.8 GB of reproducible payload retired, 0 deleted. | repo-local |
