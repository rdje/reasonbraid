...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.14.3.10` — `credential_binding_ref` SELECTS a credential and the broker's store carries **no tenant at all**. ⚠️ Measured **latent, not live**: the R5 pack is off by default and no production path registers a binding. ⭐ `.13.1.1`'s shape. ✅ **`.11.14.1.2` IS DONE (REPAIR-0233): the plan checker now has a commit-time trigger.** The runtime checker was always right and only ran when a suite ran, so six plans went unchecked for ~20 commits. ⭐ `.11.6` satisfied rather than waived — the population was measured in the PREVIOUS commit (16 parents, 45 inline keys, 29 plans) and is now **0 refused of 29**, so the gate ships green and its value is preventing the class, not finding a defect. It needs no database (0 ALTER-added keys) and ⚠️ **refuses rather than under-reports**: an unmodelled key form or an unreadable call shape is an ERROR, not a skipped edge. 🔴 Falsified at BOTH layers — and the registration's first falsification passed for the WRONG reason, because the injected edit never matched. **An injection must be shown to land**; promoted. ✅ **`.11.14.1.1`** (REPAIR-0232); ✅ **`.11.14.3.14`** (REPAIR-0231, the delegated call); ✅ **`.11.14.3.15`** (REPAIR-0230); ✅ **`.13.5`** (DOC-0043). | repo-local |
