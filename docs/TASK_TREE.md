...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.19.2` — 🔴 **the MIRROR of `.11.19`: a paragraph that ABUTS a table is swallowed by it.** `docs/tasks/PHASE-3.md`'s closing *"**Tree complete.** Phase 3 is CLOSED"* is an 11-line wrapped paragraph with no blank line above it, and the renderer emits every line as a table row padded to the header's four columns — the tree's closing statement is not prose on the page at all. ⛔ Neither table gate reaches it: `BROKEN-TABLE` keys on a blank line INSIDE a table and this is the ABSENCE of one, and `check_table_arity.sh --all` reports `0` because GFM PADS a short row, the silent direction that gate already declares. ✅ **`.11.19.1` IS DONE (REPAIR-0239): twelve verification-log rows in `docs/tasks/PHASE-2.md` had LOST their first two cells** — written well-formed by `e7a829c`, dropped by `bb42f65`, by an insert-at-top edit that re-emitted the displaced row without its date and leaf id, twelve times over twelve commits. **All 12 recovered unambiguously** by suffix-matching every well-formed row in the file's history; ⭐ the recovered leaf ids come out strictly descending, a property the match never uses. Renderer: rows whose first cell is a date **21 → 33**, rows padded with two empty cells **12 → 0**, total rows **76 both ways** — a row-count check would have reported success before the repair. 🔴 And the second half of the acceptance found **two defects in the gate shipped one commit earlier**: a FALSE POSITIVE on two adjacent tables separated by a blank line (ordinary Markdown), and a FALSE NEGATIVE on pipe-less body rows. Both fixed, falsified in situ against the pre-fix scanner, 18 arms. ✅ `.11.19` (REPAIR-0238), `.11.16` (REPAIR-0237). | repo-local |
