...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.4.2.1` — a rotation can outrun a revocation and leave a live certificate behind, which would undo the two repairs that made revocation effective; `.4.2` is censused and split into seven children | repo-local |
