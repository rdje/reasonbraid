...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.6.2` — **MCP durable delivery and transport**, the first open lane now that `.6.1` is FULLY CLOSED: the bounded dedup window retained, first delivery and monotonic cursor updates serialized, malformed state validated rather than dropped, and the documented reauthorization/reconnect/transport profile implemented with real wire evidence. ⚠️ Expect a census and a split before any repair — its acceptance asks for the failing case, the corrected case and an independent control. ✅ `.6.1.4` CLOSED (REPAIR-0286) and `.6.1` with it: the quota counts ADMITTED CALLS and the pipeline counts EFFECTS — one caller, two calls, one contribution — decided, recorded and asserted, with no behaviour changed. ✅ `.6.1.3` (0285); ✅ `.7.1.2.2.3` (0284); ✅ `.6.1.5.4` (0283); ✅ `.6.1.5.3.1` (0282) | repo-local |
