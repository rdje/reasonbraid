...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.24.1.3` — **only the EXTRACT resolution records a derivation edge**: the resolve handler has one `derivations::submit` call reading `response.extractor_version`, while GIT and BROWSE write none, although `PHASE-4.6.1` deferred them to `.6.2` and `PHASE-4.6.2` deferred the R3 render's to `.6.4`. ✅ **`.11.24.1.1.1.1` (REPAIR-0309): a receipt is earned by an offer** — the ack's *withheld now* proxy OVER-recorded, so a node could make undelivered work deletable by acking past rows it was never offered; `offered_at IS NOT NULL` replaces it, and a control that asserted the opposite of what it had just watched happen is corrected. ✅ **`.11.24.1.1.1` (0308): the tail read IS the offer.** ✅ **`.11.24.1.1.2.2` (0307): a proof of allowance must carry its own window.** ✅ **`.13.4.3.1` (0306): the revocation backfill is witnessed, and it was right** | repo-local |
