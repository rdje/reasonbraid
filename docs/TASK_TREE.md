...
| [`PHASE-2`](tasks/PHASE-2.md) | `active` | `.1.2.2` — the channel v3 cert-proof handshake + rotation (`.1.2.1` done: cert issuance at enrollment — the persisted CA + `node_certificates` + the node's stored leaf; then `.1.3` revocation → `.1.4` delegation → `.1.5` caching → `.1.6` incarnations) | repo-local |
