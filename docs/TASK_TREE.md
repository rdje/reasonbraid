...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.19` — 🔴 **a blank line ENDS a Markdown table, and one sits inside the tree's own Current Frontier table**, so its header renders empty and all 45 rows below render as literal pipe-text on a public repository. ⭐ Widening the key from "after the delimiter row" to "anywhere in the body" took the census from **1 to 51 rows across 3 files** — `a-census-is-as-wide-as-its-key` a third time in one session. ⛔ `TABLE-ARITY-RATCHET` governs these files and cannot see it: an orphaned row has no header to disagree with, which is `BOOK-LINKS`' founding shape again. ✅ **`.11.16` IS DONE (REPAIR-0237): a restated number needs a PRODUCER, not a rule.** Three candidate gates priced at **66%**, **50%** and, scoped to the staged diff, **10 of the 14 commits in 200 that add a numeral (71%)** — all DECLINED, against `.11.9`'s rejected 87%, `.11.15`'s 93% and `POSITIONAL-REF`'s accepted 9.5%; a per-numeral allowlist refused by `VISIBILITY-POLICY`'s own *"an allowlist thirty entries long teaches bypass"* against a population of 103. ⭐ **The four numerals that had actually gone stale were all a population size the named instrument enumerates on every run and never printed** — and each was stale in that instrument's own header too, a mirror of a mirror — so two `--census` flags fixed them with no gate at all. `REASON-CODE-DOC` is discharged by DELETING its numbers rather than correcting them, because a mirror that happens to agree teaches a reader it never drifted. `bare` numerals 51 → 37. | repo-local |
