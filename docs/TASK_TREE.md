...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.7.3.6.4` — **the egress ladder is compared in the direction that admits a WIDER pack than the caller asked for**: `declared >= required` is a safety floor for `sandbox_level` and a capability floor for `egress_class`, so a caller requiring `listed` is served the pack declaring `any` and no value means *do not give me a pack that can dial anywhere*. ✅ **`.7.3.6.3` (REPAIR-0268): the pair was gated in both directions at cadences ~300 commits apart** — the advertisement every commit, the behaviour only at push; `refusing_policy` is now a pure function (8 → 12 arms, 0.08 s) and the end-to-end control stays. ✅ **`.7.3.6.2` (REPAIR-0267)**: `register` INSERTed 18 columns and updated 6; split by caller after the acceptance's re-read found the table has no tenant column. ✅ **`.7.3.6.1` (REPAIR-0266): 3 of the 36 advertised policy lines are enforced.** Remaining: `.5` the undefined and misdescribed terms (**G4's open strand**) | repo-local |
