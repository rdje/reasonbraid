...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.3.4.3.1` — the node clock ahead of the server; `.3.4.5` measured both delegation representations at depths 1–3 and found ADR-009's "chain-in-envelope" carries no chain, which closes the `.3.4` family | repo-local |
