...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.14.3.10` — `credential_binding_ref` SELECTS a credential and the broker's store carries **no tenant at all**. ⚠️ Measured **latent, not live**: the R5 pack is off by default and no production path registers a binding. ⭐ `.13.1.1`'s shape. ✅ **`.11.14.1.1` IS DONE (REPAIR-0232): six suites could not start since REPAIR-0213.** `migrations/0062` added `evidence_citations` with a foreign key to `tenants` and swept only the plans naming `evidence_snapshots`. 🔴 **And the second missing child was MINE** — `0067` added `reference_registrations` with keys to both `resource_references` and `tenants`, and I swept the 21 plans naming the first and not the plans naming the second, in the same session that promoted the rule. **Knowing the rule is not applying it: the sweep must be keyed on the new table's OWN constraints, computed rather than recalled.** ⚠️ The leaf also corrects its own first number — it opened saying TEN, from a grep that matched a row-count tuple and a bare list entry; the tightened instrument parses the `delete_tables` arrays and gives **16 tables, 27 plans, 6 refused → 0**. ✅ **`.11.14.3.14`** (REPAIR-0231, the director's delegated call); ✅ **`.11.14.3.15`** (REPAIR-0230, the census correction); ✅ **`.13.5`** (DOC-0043, the register column); ✅ **`.11.14.3.7`** (REPAIR-0229); ✅ **`.11.14.3.5`** (REPAIR-0228). | repo-local |
