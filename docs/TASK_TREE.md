...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.7.2.10` — 🔴 **`AcquisitionError.kind` is a second refusal vocabulary and nothing documents it.** It is NOT a §9.8 reason code (the struct carries `kind`, not `code`), so `REASON-CODE-DOC` is right to exclude it — but a client branches on these strings and the book shows one only by accident, in a JSON example. ⛔ The 93 mapped literals in the server are a POPULATION: classify first. ✅ **`.7.2.7` IS DONE (REPAIR-0249): a refusal wired end to end and constructed by nothing** — `GitError::TimedOut` declared, given a Display message and mapped to the wire string `timed_out`, with no construction site, while `max_time` was read by no code path and `gix` was handed an interrupt flag nobody could raise. ⭐ The repair bounds the TRANSFER, not the caller's wait. ✅ *Before allocation* narrowed with its measurement — all five other ceilings trip after `receive`, so consumption is bounded **in seconds, not bytes**. ⛔ Decode brake and overflow guard both DECLINED on measurements. ✅ `.3.4.3.1.1.1` (DOC-0047), `.11.2.5` (0248), `.11.18.2` (0247). | repo-local |
