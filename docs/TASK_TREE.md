...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.24.1.1.2.1.1.1` — **revoking a grant records that it happened and not when**: `revocation.rs` writes `SET status = 'revoked'` and `authority_grants` has no `revoked_at`, so anything deriving from the grant cannot ask when authority was withdrawn. ⚠️ A write on the authority core, whose transaction shape is `.3.3.4.8`'s. ✅ **`.11.24.1.1.2.1.1` (REPAIR-0302): an expired row prunes on its own window, a revoked one has no clock to prune by.** ⭐ A retention window measures time in the STATE being retained — and that rule gives two different answers here. ✅ **`.11.24.1.1.2.1` (0301): a cursor acknowledgement stops receipting rows the tail withheld** — `.4.2.4` had already recorded the harm model, that column being the prune's DELETE predicate. ✅ **`.11.24.1.1.2` (0300): a command cannot outlive the authority that admitted it**, and ⛔ the max age is DERIVED, never chosen — the grant's own `expires_at`. 🔴 **`.11.24.1.1` (0297): `delivery_state` published `acknowledged` for §10.6's `transport_received`**, the one state the suite never asserted. 🔴 `.11.24.1` (0296) adjudicated 18 deferrals and 12 goal items and found **seven stranded** | repo-local |
