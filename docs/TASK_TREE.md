...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.2.2` — three doctrine enforcers write scratch off the repository volume where `STORAGE-LOCALITY`'s `*.rs` scan cannot see them. It and `.3.5.3` are `.11.9.1`'s two findings: that leaf censused the 114 uncited review records (491 clauses), REJECTED its own small-fan-out ranking on the measurement and built the clause ledger. `.3.5.3` is closed — the inbox INSPECTION returned another tenant's rows, the fourth verb its record named and the one a mutations-only census left behind — and left `.3.5.4` owning the read census it could not finish | repo-local |
