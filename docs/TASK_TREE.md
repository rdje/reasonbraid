...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.24.1.1.2.2` — **a dispatched work item's budget reservation expires TEN MINUTES after dispatch and nothing bounds delivery to that window**: `budget.rs` stops counting an expired active hold, so a node polling at minute eleven runs against a ceiling that has re-lent its allowance. ⚠️ NOT a claim ten minutes is wrong — `.11.6` forbids revising the number before measuring the population; three honest answers. ✅ **`.11.24.1.1.2.1.1.1.1` (REPAIR-0304): no column, because the pattern it would complete is one nobody reads** — the leaf's own opening argument withdrawn by the census it demanded. ⭐ Promoted: *a column is not missing until something needs it — census the READERS, not the tables.* ✅ **`.11.24.1.1.2.1.1.1` (0303): revoking an authority records when**, written by the same statement that changes the status, so the row and the audit trail agree by construction. ✅ **`.11.24.1.1.2.1.1` (0302): each terminal prunes on its own clock.** ✅ **`.11.24.1.1.2.1` (0301): the cursor ack stops receipting rows the tail withheld.** ✅ **`.11.24.1.1.2` (0300): a command cannot outlive the authority that admitted it** | repo-local |
