...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.16` — `DOCTRINE_ENFORCEMENT.md` calls itself the registry's human mirror and its `REASON-CODE-DOC` row's population had drifted since `.9.2.1.1`; ⛔ the population must be CLASSIFIED before any rule is proposed. ✅ **`.11.17` IS DONE (REPAIR-0236): a positional reference is exact only if a reader can resolve it.** A bare `profiles.rs` with a five-digit line named a 606-line source AND an 11,154-line suite. ⭐ The leaf's 29 reconciled exactly once the UNIT was named — 29 distinct, 47 occurrences — and all 47 are discharged to **0**. ⭐ `POSITIONAL-REF` ships calibrated across 200 commits before it was proposed (`.11.6`): **48 of 415 added references ambiguous (11.6%), 19 of 200 commits blocked (9.5%)**, against the 87% and 93% that got `.11.9`'s and `.11.15`'s candidates rejected for teaching bypass. ⛔ It gates AMBIGUITY, not DRIFT — five references had already drifted and were confirmed by SYMBOL, not by line. 🔴 It flagged its own registry row and then its own leaf prose, and 🔴 the falsification's `git checkout --` destroyed three uncommitted discharges, correcting the knowledge note that recommended it. ✅ **`.11.18`** (REPAIR-0235); ✅ **`.11.14.3.10`** (REPAIR-0234, closing the `.11.14.3` family); ✅ **`.11.14.1.2`** (REPAIR-0233). | repo-local |
