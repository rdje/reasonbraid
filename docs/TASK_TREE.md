...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.24.1.3.1` — the R3 snapshot and one derivation edge per chunk; the parent now EXISTS and the worker sends it, so the blocker has narrowed to the server struct not taking the field. ✅ **`.11.24.1.3.1.1` (REPAIR-0311): the render's parent was its own derivation** — `parent_digest` digested the concatenated chunk texts and the worker derives exactly one chunk, so parent == chunk digest on every render, and an empty render's parent was the empty string; the worker now carries `page.content()`, behind a pure function a control can guard without driving Chrome. 🔎 A real-browser control fails at HEAD, measured on both sides → `.11.25`. ✅ **`.11.24.1.3` (0310): the missing derivation edges are a SYMPTOM** — two packs produce evidence the store cannot hold. ✅ **`.11.24.1.1.1.1` (0309): a receipt is earned by an offer.** ✅ **`.11.24.1.1.1` (0308): the tail read IS the offer** | repo-local |
