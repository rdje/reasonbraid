...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.7.1` — **authenticated resource and resolver ownership**: `resolver_capabilities` has no tenant column and a single-column key, so any tenant administrator can create a site-global resolver row (`.11.9.1.1.1`). ✅ The surface is NARROWER than when last looked at — `.7.3.6.2` closed the replace half, so what remains is creation. ✅ **`.7.3.6` CLOSES with all five children (REPAIR-0266–0270): 36 advertised policy lines, `enforced` 6 · `unverified` 9 · `vacuous` 21 — `undefined` 0, `misdescribed` 0.** 🔴 **G4's open strand is discharged**: R3 advertised `vm_container` while running an ordinary child process, and a caller requiring it was served the pack — corrected to `process`. ⭐ `archive_policy`, `egress_class: "listed"` and `allow-bounded` were DEFINED from the producer, not corrected. Three product defects repaired on the way; one control moved from push-cadence to commit-cadence | repo-local |
