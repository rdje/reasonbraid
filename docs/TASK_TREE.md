...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.14.3.1` — 🔴 **a shipped built-in profile advances onto a step that records nothing.** `git grep -n '"assess"' -- crates/reasonbraid-server/src` returns **1 hit, the vocabulary constant**, while `evidence_review` and `policy_proposal` both declare the step. ✅ **`.11.14.3` IS DECIDED (DOC-0037), and the HOLD was wrong.** The leaf was opened saying the frozen roadmap "settles it neither way" — written from §12.7 and §5.2.2. **§13.2 was not read**, and it specifies the join as steps 2, 5 and 6 of the canonical deliberation flow. A "the spec is silent" claim is a census over the spec, and that one covered two sections of twenty-five. **Decision: the deliberation flow owns the evidence chain**, joined where §13.2 names — `assess` writes a `claim_assessments` row keyed by a membership-checked claim digest over a CITED snapshot, and a contribution's `EvidenceRef` resolves to a `resource_references` row (the same key). Four alternatives rejected, including a `thread_id` column on the content-addressed tables. Split into `.1`–`.3`, checked against four mechanisms. ✅ **`.11.14.2` IS DONE (REPAIR-0215): an assessment is read by the tenant that AUTHORED it**, and it CORRECTED `.11.14`'s own record — `claim_assessments_replay_idx` carries the AUTHOR, so that table was never shared. ✅ **`.7.4.3` IS DONE (REPAIR-0214): the retention sweep is a site act on the server's clock.** RED was HTTP 200 and `{"tombstoned":2}`. ✅ **`.11.14.1` IS DONE (REPAIR-0213): the evidence reads are bound to the CITING TENANT**, through `evidence_citations` written on the replay as well as the insert. | repo-local |
