...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.6.1.5.3` — bind the 32 lifecycle READS, HTTP and MCP together, the last open half of `.6.1.5`. ⛔ The population is PINNED BY NAME in the leaf: `.6.1.5.2` removed all nine tables from the census that was watching them, because a table with a tenant column is invisible to `census_registry_read_reach.py` while its readers stay site-wide. ✅ `.6.1.5.2.1` CLOSED (REPAIR-0280): eleven lifecycle verbs now require the caller to own the record — and ARM 4 passed for an unrelated reason until falsification showed the git ref was written before the 400. ✅ `.7.1.2.2.2` CLOSED (REPAIR-0279); ✅ `.6.1.5.2` CLOSED (REPAIR-0278); ✅ `.7.1.2.2.1` CLOSED (REPAIR-0277) | repo-local |
