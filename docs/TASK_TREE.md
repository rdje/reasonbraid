...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.8` — the HTTP surface and the book have drifted; `.3.5.1` refused its own proposed repair and found a node id owned by another tenant answered 500, one arm away from answering with a certificate | repo-local |
