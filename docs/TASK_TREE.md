...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.9.1.3.2` — tranche 4b of the clause reconciliation. `.11.9.1.3` is sized at 10,519 characters (4.12x the proved size) and split five ways on the derived boundary with one declared deviation; `.11.9.1.3.1` is closed by REPAIR-0180 with 26 clauses (22 `owned`, 2 `attach`, 2 `unowned`). 🔴 **It corrected a number this activity itself published** — a fixture count of "four places" is seven distinct test functions, and `git show` at the original commit proves it was wrong when written, not overtaken. 🔴 It also found `corrections.rs::authority_holds` admitting any enrolled caller who names any active grant id, and a deterministic `review_id` primary key that makes a second review for a (publication, trigger) pair impossible for ever. New leaf `.11.12` owns `rb-server` migrating before it validates. Row 1b is `.11.12` | repo-local |
