...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.6.2.3` — **a malformed MCP dedup window is silently replaced with an empty one** (`unwrap_or_default()`, one expression): with the column set to a JSON object, a delivery the state had already recorded replays and is ACCEPTED. Silent, total and self-erasing — the next UPDATE overwrites the malformed value. ⚠️ Fail-closed has a priced cost: one corrupt row would stop that subscription entirely. ✅ `.6.2.2` CLOSED (REPAIR-0289): the cursor is a high-water mark and the late delivery is still APPLIED — a clamp, not a refusal. ✅ `.6.2.1` (0288): the dedup window keeps the newest ids. ⚠️ `.6.2.4` (the transport BUILD) remains, ranked last. ✅ `.6.1` is FULLY CLOSED | repo-local |
