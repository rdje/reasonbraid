...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.7.1.2.2` — the routing journal: `routing_resolutions` and `routing_recommendations` each have ONE reader (`list_*`) and no predicate of any kind, so every tenant reads every tenant's routing trail. ⚠️ A DISCLOSURE path — `routing::resolve` reads neither — so DOC-0029's *tenant-bound read* remedy fits exactly. ✅ `.7.1.2.1` CLOSED (REPAIR-0273): **the takeover was OBSERVED before it was repaired** — Alice's thread, in Alice's own tenant, ran Mallory's steps — and `workflow_register` is now a site capability (`0071`, following `0063`), decided by the code's own words: `register`'s doc comment already said *the operator's verb*. ✅ `.7.1.2` CLOSED (DOC-0066): the seventeen adjudicated **4 · 1 · 2 · 10**; `.6.1.5` re-scoped from one table to ten | repo-local |
