...
| [`PHASE-7`](tasks/PHASE-7.md) | `active` | `.1.3` — the tenant isolation + the quotas/abuse (`.1.1` done: ADR-034 accepted; `.1.2` done: the mTLS workload identity — the TLS 1.3-only config pair + the offline roundtrip proof) | repo-local |
