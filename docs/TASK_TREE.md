...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.6.2.4` — the MCP reauthorization/reconnect/transport profile has no wire evidence, and it is the leaf that would WIRE THE FIRST CALLER onto `mcp_listen`. ⚠️ A BUILD, not a repair: its acceptance must quote the documented profile from its source BEFORE implementation. ✅ `.6.2.1`–`.6.2.3` CLOSED (0288, 0289, 0290) — the dedup window keeps the newest ids, the cursor is a high-water mark with the late delivery still applied, and a malformed window fails CLOSED while keeping the evidence. 🔴 `.6.2.3.1` CORRECTS my own published claims: the caller census found NOTHING calls `mcp_listen`, so the lane's defects are LATENT, not live — true of the code, overstated about the deployment. The repairs stand; they now land before the first caller does. ✅ `.6.1` is FULLY CLOSED | repo-local |
