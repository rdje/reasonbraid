...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.6.1.5.1.1` — 🔴 **two lifecycle verbs whose tenant claim binds nothing in the profile this repository runs**: `.6.1.5.1` wrote the same `rls::with_tenant_claim` check, ran it, and watched a foreign tenant receive 200, because the dev superuser bypasses RLS and `FORCE ROW LEVEL SECURITY` does not reach it. ✅ `.6.1.5.1` CLOSED (REPAIR-0275): an approval is bound to its proposal's tenant by an explicit predicate; Mallory's approval of Alice's proposal had returned 200 and been stored. ✅ `.6.1.5` CLOSED (DOC-0071): the policy LIBRARY is site-wide by design, the nine LIFECYCLE tables tenant-owned by omission; 47 SQL sites, not six | repo-local |
