...
| [`PHASE-2`](tasks/PHASE-2.md) | `active` | `.1.2` — the certificate lifecycle + channel v3 (`.1.1` done: ADR-006/007 accepted — the spike-measured project-local CA model; then `.1.3` revocation → `.1.4` delegation → `.1.5` caching → `.1.6` incarnations) | repo-local |
