...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.9.1.1` — tranche 2 of the clause reconciliation, 14 records, now that the ledger and its instrument are proved. `.11.9.1` censused the 114 uncited review records (491 clauses) and REJECTED its own small-fan-out ranking on the measurement; both of its findings are closed. `.3.5.3`: the inbox INSPECTION returned another tenant's rows, the fourth verb its record named, left behind by a mutations-only census — `.3.5.4` owns the read census it could not finish. `.11.2.2`: the doctrine enforcers' own scratch was off-volume and `STORAGE-LOCALITY` could not see it, now extended to shell and Python, firing on 0 today and on all 7 historically | repo-local |
