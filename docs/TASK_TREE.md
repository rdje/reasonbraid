...
| [`PHASE-8`](tasks/PHASE-8.md) | `active` | `.3.2` — the MCP SDK pin (opened at the Phase-7 close; the `.1`/`.2` lanes COMPLETE; `.3` done: the census → decomposed; `.3.1` done: ADR-024 accepted) | repo-local |
