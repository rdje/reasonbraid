...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.6.5` — the book documents NO MCP surface at all while the surface is shipped and was repaired five times in `.6.1`; the book is the director's only window, so this is drift in the direction that matters most. ✅ **`.6.2` FULLY CLOSED** with `.6.2.4` (REPAIR-0293): the five-step reconnect ritual runs end to end against an upstream that cuts a chunked body mid-stream, with the upstream's own record of the steps it saw and **7/7 in-situ mutations caught**. 🔎 **An offered replay is not a covered gap** — a boolean `upstream_replay` reported continuity for an upstream replaying from 50 while we hold 11. ⛔ Not an SDK/conformance claim (`.6.6`), and there is no operator-facing gateway (`.6.7`). ✅ `.13.4.1` (0291); ✅ `.6.2.1`–`.6.2.3` (0288–0290); ✅ `.6.1` FULLY CLOSED | repo-local |
