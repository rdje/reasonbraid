...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.24.1.1` — **the §10.6 delivery ladder ships three of its five states** and the offline-delivery expiry does not exist; the migration's own header is honest about it and the goal line is not. 🔴 **`.11.24.1` (REPAIR-0296) adjudicated 18 deferral pairs and 12 goal items and found SEVEN STRANDED**, five of them unknown before: the delivery ladder, the GIT/BROWSE derivation edges, `PresenceState::Busy` (advertised, never constructible), the adapter SDK's missing acquisition trait, and the release tool's missing re-sign. ⚪ 2 of 18 are artifacts and NEITHER can be tightened away — `rides` also means *uses*, and a bare `.2.3` in `PHASE-8` reaches a `PHASE-8` leaf that exists. ✅ `.11.24` (0295) shipped the census and the `LEAF-ID-UNIQUE` gate; ✅ `.6.5` (0294) found nothing serves the MCP tools (`.6.8`); ✅ `.6.2` FULLY CLOSED with `.6.2.4` (0293) | repo-local |
