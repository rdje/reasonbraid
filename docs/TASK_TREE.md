...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.3.5.3` — `inspect_node_inbox` reads another tenant's rows, the fourth verb `R-31-32-1` named and the one a mutations-only census left behind; found by `.11.9.1`, which censused the 114 uncited records (491 clauses), REJECTED its own small-fan-out ranking on the measurement, built the clause ledger and split the rest into six tranches. Its other live finding is `.11.2.2`: three doctrine enforcers write scratch off the repository volume where `STORAGE-LOCALITY`'s `*.rs` scan cannot see them | repo-local |
