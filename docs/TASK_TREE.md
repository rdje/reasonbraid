...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.24.1.1.1` — the two UNDERIVED §10.6 ladder states: `offered` (the server knows when a poll response carried a row and records nothing) and `acknowledged` (⛔ a DESIGN commitment before an implementation — §10.6 defines it as *explicit per event type* and the channel has one cursor ack). ✅ **`.11.24.1.1.2.2` (REPAIR-0307): a proof of allowance must carry its own window** — a work item's reservation held for ten minutes and the ledger stopped counting it at `expires_at`, while the reference the node verifies carried `issued_at` and no window, so §14.3 step 4 ran against a proof it could not date; `expires_at` now rides the reference from the same variable the `INSERT` binds, and `applicable_at(now)` refuses a lapsed hold before the adapter is contacted. ⚠️ A deliberate wire break. ✅ **`.13.4.3.1` (REPAIR-0306): the revocation backfill is witnessed, and it was right** — 11 rows whose answers are known before the migration runs, and 9 mutations turn the control red 6 times; ⚠️ the 3 that cannot are recorded rather than faked. ✅ `.13.4.3` (0305): 22 claims re-derived, 20 hold. ✅ `.11.24.1.1.2.1.1.1.1` (0304): no column — the pattern it would complete is one nobody reads | repo-local |
