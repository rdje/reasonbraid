...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.4.7.4` — **the last open child of `.11.4.7`; closing it closes blocker C2.** G4 and G5, the two subtraction-shaped gates (4 verdict claims). ✅ **`.11.4.7.3` (REPAIR-0264)**: G3's seven claims re-derived — **3 stand · 1 narrows · 3 had to be re-earned**, all three since discharged. 🔴 Its evidence is cited by test POSITION: `policy.rs` went 11→14 and the correction clause's only pointer now names a publication test; three more are unresolvable because the suite's count equalled the highest index cited. Three of five gate records are now re-derived, two remain. ✅ **`.4.2.3.1` (REPAIR-0263)**: the lease was written by the process clock and read by the database's — **660.0 s** granted against a published **60 s** under a driven skew; the database clock now owns the column at all five sites. ✅ **`.11.2.6` (REPAIR-0262)**: the acceptance gate read **1** of this tree's **194** checklists and hard-gated a vocabulary the project does not use. | repo-local |
