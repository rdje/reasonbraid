...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.3.4.3.1.1` — the server does not have one clock; `.3.4.3.1` censused the clock population and found three clocks, two node-side comparisons failing in opposite directions, and split into three children | repo-local |
