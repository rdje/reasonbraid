...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.4.2.4` — `ack` writes on a pre-check the way `events` used to; `.4.2.1`/`.4.2.6`/`.4.2.2`/`.4.2.3` closed the rotation ordering, the proof ladder, proof replay and the lease-renewal window | repo-local |
