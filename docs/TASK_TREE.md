...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.3.4.3.1.2` — let the node evaluate server instants in the server's terms; `.3.4.3.1.1` measured the server's two clocks and found the stored certificate expiry was never read from the certificate | repo-local |
