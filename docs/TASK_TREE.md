...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.24` — **two `done` Phase-8 leaves whose GOAL line outran their own DONE list**, found in consecutive slices, and nothing in this repository compares a goal to its receipt. Ranked first because it sizes the class before a feature slice is spent on one instance. 🔴 **`.6.5` (REPAIR-0294) found the consequence: NOTHING SERVES THE MCP TOOLS** — six implemented, authorized, live-tested tools reachable by no client, because *the transport rides `.3.4`* and `.3.4` closed without one (`.6.8`). ✅ The book gained two MCP chapters, the surface and the listen gateway. ✅ **`.6.2` FULLY CLOSED** with `.6.2.4` (REPAIR-0293): the five-step reconnect ritual across a real disconnect, **7/7 in-situ mutations caught**, and *an offered replay is not a covered gap*. ⛔ No SDK/conformance claim (`.6.6`); no operator-facing gateway (`.6.7`) | repo-local |
