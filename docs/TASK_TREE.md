...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.14.3.8` — `POST /v1/assessments` has no citation gate, while the `assess` step refuses an uncited snapshot with the stated reason that "an assessment would be a way to learn that a snapshot exists". Both call the SAME `claims::submit`; the census is 4 gate sites, 3 of them READS and exactly one write path. ✅ **`.11.14.3.3` IS DONE (REPAIR-0219): a replay key is an addressing scheme, and two of its columns had to become server-set.** 🔴 The key was `(claim_id, snapshot_id, assessment, author)` and `author` is a CALLER LABEL, so `claims::submit`'s pre-check handed back rows the caller never wrote — **twice over, each reproduced RED first**: a caller naming a thread's minted claim digest got **the deliberation's own `assessment_id`**, and a second TENANT presenting the first's `author` label got **that tenant's**. ⭐ **The second was found only by AUDITING my own published claim** against `docs/CLAIM_VERIFICATION.md`: I had published "bounded to one tenant" from READING the SQL, and the pre-check carries no tenant predicate at all. It also corrects a false premise in `migrations/0064`, which justified its design on `author` separating two tenants and called `author` unauthenticated eight lines later. `0066` adds `claim_namespace` AND `authored_by_tenant` to the key and the pre-check; `0064` is left byte-unchanged (checksums are load-bearing; decisions supersede rather than mutate). ⚠️ Left open and stated: two principals inside ONE tenant can still alias — not a disclosure, since the authoring gate already admits both. The rows are left with no backfill for the third time in this family, and the route is kept and labelled rather than removed, digest-minted or filtered. ✅ **`.11.14.3.2`** (REPAIR-0218); ✅ **`.11.14.3.1`** (REPAIR-0217); ✅ **`.11.14.3`** (DOC-0037). | repo-local |
