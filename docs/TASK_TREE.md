...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.4.2.3.1` — **`node_leases.lease_expires_at` is written from the process clock and read from the database clock**, so the lease's real duration carries the skew; the column has readers on both clocks. Decide which owns it, or carry the skew term in the book. ⭐ `.3.5` is now fully discharged (`.3.5.1`–`.3.5.5`). ✅ **`.11.2.6` (REPAIR-0262)**: the acceptance gate read **1** of this tree's **194** checklists and hard-gated a vocabulary the project does not use — both halves from one cause, an inert control. Scope is now the deepest leaf the commit closes; families are a census-derived `.doctrine/` seam; calibrated 57.7% → **20.6%**; it gained the `--self-test` it never had, and `--debt` counts **73 of 281** closed leaves rather than backfilling them. ✅ **`.3.5.5` (REPAIR-0261)**: `GET /v1/nodes/presence` asked nobody who was calling. ✅ **`.3.5.4` (REPAIR-0260)**: the read census could not see 34 of its own routes — 24 published, **58** real. | repo-local |
