...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.24.1.1.1.1` — the cursor acknowledgement keys on what is WITHHELD NOW, a proxy for *was this row carried*; `migrations/0078` now records the fact the proxy stood in for, and the leaf must answer the pre-`0078` NULL cohort rather than discover it. ✅ **`.11.24.1.1.1` (REPAIR-0308): the tail read IS the offer** — §10.6's `offered` had no producer, so a row handed to a transport read `queued`; the mark now rides the same statement as the tail read, write-once, scoped to the rows the tail returns, with the count and the backfill both REFUSED and `acknowledged` deferred on a measurement. ✅ **`.11.24.1.1.2.2` (0307): a proof of allowance must carry its own window.** ✅ **`.13.4.3.1` (0306): the revocation backfill is witnessed, and it was right.** ✅ `.13.4.3` (0305): 22 claims re-derived, 20 hold | repo-local |
