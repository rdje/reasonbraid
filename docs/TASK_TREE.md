...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.7.1.2.2.2` — a suite that asserts over a whole table must purge it: `routing evaluation` in that order fails `evaluation` 2/1, the reverse passes both, because `tests/evaluation.rs` asserts the content of `evaluation_trials` and never purges it. ⛔ `.7.1.2.2.1`'s FK gate cannot catch this — different class. Then `.6.1.5.2.1` (the five lifecycle writes that act on another tenant's publication) and `.6.1.5.3` (the 32 reads). ✅ `.6.1.5.2` CLOSED (REPAIR-0278): the nine lifecycle tables store the tenant the write already derived — four from the caller, five from their PARENT — backfill coverage MEASURED at `policy_proposals` 1/3, `policy_projections` 0/2, the rest 1/2. ✅ `.7.1.2.2.1` CLOSED (REPAIR-0277): the fixture-plan gate could not see `ALTER TABLE … ADD COLUMN … REFERENCES`, so 22 purge plans were stale and 20 suites could not start | repo-local |
