...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.25` — **a real-browser control fails at HEAD on this host** (`a_real_navigation_deadline_stops_the_browser_and_origin`), measured on BOTH sides and identical at the parent commit; the RENDER's contract holds and what is unconfirmed is the CLEANUP fact. ✅ **`.11.24.1.3.1` (REPAIR-0312): a render is evidence, and its worker must describe the document it sent** — the snapshot is the serialized page, one edge per chunk, and a worker whose declared digest is not its document's is refused; falsified four ways, one verdict landing on a guard the leaf did not write. ✅ **`.11.24.1.3.1.1` (0311): the render's parent was its own derivation.** ✅ **`.11.24.1.3` (0310): the missing derivation edges are a SYMPTOM.** ✅ **`.11.24.1.1.1.1` (0309): a receipt is earned by an offer** | repo-local |
