...
| [`PHASE-8`](tasks/PHASE-8.md) | `active` | `.1.1` — the census + ADR-026 (opened at the Phase-7 close: the `.1` block LIFTS — the trust + the compatibility contracts ship + are measured; decomposed `.1.1` ADR-026 → `.1.2` the visibility/recruitment → `.1.3` the portable cards → `.1.4` the receipts) | repo-local |
