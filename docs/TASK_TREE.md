...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.14.3.12` — two acquisition arms DISCARD whether the evidence was persisted (`let _ = crate::snapshots::submit(…)`), so resolving a pinned reference whose page drifted returns a receipt and **no snapshot**, silently. ⛔ Do NOT assume the answer is to fail the resolution: the acquisition DID happen. ✅ **`.11.14.3.6` IS DONE (REPAIR-0224): a reference's `expected_digest` names its bytes.** 🔴 The pin was recorded and read by NOTHING — the snapshot store asked `SELECT EXISTS … WHERE resource_id = $1` — so a reference pinned to one digest accepted a snapshot of entirely different bytes, falsified against the reverted production file at `200 {"replay":false,"snapshot_id":"snp_01a0aec5…"}`. ⭐ Enforcement is what makes `.11.14.3.2`'s pair key MEAN something: that leaf made `(locator, digest)` the identity so a changed page would be a second reference, and with no checkpoint the distinction was preserved nowhere. ⚠️ **The plural is relocated, not forbidden** — an UNPINNED reference still holds every version, and the control asserts 2. ⛔ The refusal quotes neither digest. ✅ **`.11.14.3.4`** (REPAIR-0223); ✅ **`.11.14.3.8`** (REPAIR-0222); ✅ **`.11.14.3.3`** (REPAIR-0219); ✅ **`.11.14.3.2`** (REPAIR-0218); ✅ **`.11.14.3.1`** (REPAIR-0217). | repo-local |
