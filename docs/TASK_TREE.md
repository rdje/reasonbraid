...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.7.2.9` — 🔴 **A pack can expand far beyond its wire size and only the clock stops it.** `.7.2.7` measured that `gix-pack 0.74.2` has no decompression-ratio guard but allocates with `try_reserve`, so one absurd entry errors rather than aborting; the residual is a pack staying under the allocator's limit per entry while expanding in aggregate. ⚠️ Unreproduced. ✅ **`.7.2.10` IS DONE (REPAIR-0250): a second refusal vocabulary — 41 strings a client branches on, documented nowhere.** `acquisition_error.kind` is NOT a §9.8 code (**0** shared strings), so `REASON-CODE-DOC` was right to exclude it. ⭐ Derived from the producers, and **three keys were wrong before the fourth was right** — all pinned as arms. The instrument's 41 agrees with a hand derivation reached another way. Calibrated **0 of 200**, real rather than survivorship. ✅ `.7.2.7` (0249), `.3.4.3.1.1.1` (DOC-0047), `.11.2.5` (0248). | repo-local |
