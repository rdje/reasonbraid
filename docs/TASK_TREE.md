...
| [`PHASE-7`](tasks/PHASE-7.md) | `active` | `.2.1` — the census + ADR-027 (the `.1` lane COMPLETE; `.2` done: the census → decomposed `.2.1` ADR-027 → `.2.2` the disclosure → `.2.3` the SBOM/signing → `.2.4` the public-enrollment contract) | repo-local |
