...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.4.1.3` — what revocation does to a live lease; `.4.1`'s census of five mechanisms found one confirmed defect (a token that expired unused locked its node out for good, repaired at `.4.1.1`), refuted the rotation/revocation worry outright, and left two open | repo-local |
