...
| [`PHASE-8`](tasks/PHASE-8.md) | `active` | `.4.1` — the SDK contract (opened at the Phase-7 close; **the `.1`/`.2`/`.3` lanes COMPLETE** — the `.3` MCP surface: ADR-024 + the rmcp pin + the read/write tools + the listen-stream durability + the qualified gate + the live tool-path roundtrip; `.4` the adapter/resolver SDK census → decomposed `.4.1`–`.4.4`) | repo-local |
