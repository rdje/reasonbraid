...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.9.1.1.2` — tranche 2b of the clause reconciliation, four records. `.11.9.1.1` measured tranche 2 at 2.8x tranche 1 and split it on that number; `.1.1.1` reconciled the first six records (27 clauses) and found that tranche 1 had recorded three `attach` rows and attached none — `SIGNOFF-REPAIR.11.9`'s own mechanism, live inside the instrument built to stop it, now fixed for all seven. `.11.10` is closed by REPAIR-0172: `regions::route` reported any `sqlx::Error` as an undeclared-region verdict for a region that IS declared, and now returns a typed storage failure while every existing refusal keeps its exact wording | repo-local |
