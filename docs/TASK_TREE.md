...
| [`PHASE-7`](tasks/PHASE-7.md) | `active` | `.1.3.1` — the RLS defense-in-depth (`.1.1` done: ADR-034; `.1.2` done: the mTLS identity; `.1.3` done: the census — decomposed `.1.3.1` RLS → `.1.3.2` quotas → `.1.3.3` the quarantine-evidence rule) | repo-local |
