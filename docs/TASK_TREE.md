...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.26` — one of `COMMIT.md`'s four pre-push gates failed once in fifteen and its evidence was discarded by the `tail -4` it was run through. ✅ **`.11.28` (REPAIR-0317): two enrollment expectations counted quota rows, and the row that moved them was three days old** — the first push's `rust` workflow went red; the expectations now NAME the rows, because a count has no producer. ✅ **`.11.27` (0316): a self-test that pins one host's collation is not ground truth** — the first push's `doctrines` workflow went red while green locally. ✅ **`.11.24.1.2` (0315): §10.2's sixth presence state is reachable.** ✅ **`.11.25.1.1` (0314): a censored observation is not a duration** | repo-local |
