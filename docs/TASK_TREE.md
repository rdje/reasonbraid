...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.4.2.7` — the node's hex decoder panics on a non-ASCII response; `.4.2.1`/`.4.2.2`/`.4.2.3`/`.4.2.4`/`.4.2.6` closed the rotation ordering, proof replay, the proof ladder and both check-vs-commit windows | repo-local |
