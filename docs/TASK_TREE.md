...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.18` — `GAP-CLAIM-CENSUS` reads a shell comment inside a fenced block as a section heading. 🔎 **The dangerous direction is the SILENT mirror of the one that fired**: a false positive blocks a commit and is fixed in minutes, while a fenced `#` ABOVE a claim lets an unbacked claim discharge against evidence that is not its own. ⛔ **7 instances in 2 files, 2 of them pre-existing inside `.11.14.3`'s own census** — unreported because the gate is staged-scoped. ✅ **`.11.14.3.10` IS DONE (REPAIR-0234): a credential selector lived on a CONTENT-addressed row.** `resource_references` is keyed `UNIQUE (original_locator, expected_digest)`, so a second tenant replaying the pair inherited a binding it never named and its resolve attached the FIRST tenant's credential — ROADMAP §16.3 invariant 5. The selector moved to `reference_registrations` and `migrations/0069` DROPPED the column, so the unbound read has none to leak. ⭐ **The fourth instance of one shape** (citations, assessments, reference reads, now a means of ACCESS) — promoted as `a-shared-row-may-not-hold-a-tenant-decision`. ⭐ The repair is wider than a denial: the binding is a RANKING input, so a tenant that named none is not routed to the credential pack at all. ⚠️ The naming half — the broker's namespace is global — is `.11.14.3.10.1`, blocked on an authorization vocabulary `GrantAction` does not have. ✅ **`.11.14.1.2`** (REPAIR-0233); ✅ **`.11.14.1.1`** (REPAIR-0232); ✅ **`.11.14.3.14`** (REPAIR-0231). | repo-local |
