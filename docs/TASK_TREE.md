...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.25.1` — **something holds the browser's stderr for 16,427 ms after its owned group is reaped in 181 ms**, 1.64× the whole cleanup budget, then releases it; the holder is deliberately NOT named yet. ✅ **`.11.25` (REPAIR-0313): the cleanup budget now says where it went, and the wait I wanted to shorten is the DETECTOR** — the repair reached was withdrawn by the leaf's own acceptance clause, because `a_render_refusal_survives_an_unconfirmed_cleanup` builds a `setsid` escapee on purpose. ✅ **`.11.24.1.3.1` (0312): a render is evidence, and its worker must describe the document it sent.** ✅ **`.11.24.1.3.1.1` (0311): the render's parent was its own derivation.** ✅ **`.11.24.1.3` (0310): the missing derivation edges are a symptom** | repo-local |
