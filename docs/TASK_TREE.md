...
| [`PHASE-8`](tasks/PHASE-8.md) | `active` | `.2.1` — the census + ADR-025 (opened at the Phase-7 close; the `.1` lane COMPLETE; `.2` done: the A2A census → decomposed `.2.1` ADR-025 → `.2.2` the pin → `.2.3` the facade → `.2.4` the qualification) | repo-local |
