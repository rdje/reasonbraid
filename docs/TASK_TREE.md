...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.9.1.2` — tranche 3 of the clause reconciliation. `.11.11` is closed by REPAIR-0176 and registered as `ATTACH-LANDED`, the registry's 24th named doctrine (the enforcer's own top-level count stays 18 — it runs inside `PROJECT-SPECIFIC`): an `attach` clause whose owning leaf's own section does not NAME its record now blocks the commit. ⭐ Measured at four tranche closes by two independent routes before registering — **3 of 3** breaching at tranche 1's close, then 0 of 7, 0 of 15 and 0 of 26 — the `REASON-CODE-DOC` shape rather than the backlog shape `.11.9` rejected at 114 of 131. `.11.9.1.1.3` is closed by REPAIR-0175: tranche 2c classified 38 clauses and completed tranche 2 at 14 records and 84 clauses, and found that `check_table_arity.sh` disagrees with the renderer that publishes this book. Row 1b is `.3.5.4` | repo-local |
