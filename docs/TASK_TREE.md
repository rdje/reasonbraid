...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.4.1.3.1` — the server hands NEW work to a node it has revoked; `.4.1.3` found revocation did nothing at all to a running node (it renewed its own lease for ever) and bounded it to the 60 s lease tail | repo-local |
