...
| [`PHASE-8`](tasks/PHASE-8.md) | `active` | `.3.5` — the MCP write-half profile (opened at the Phase-7 close; the `.1`/`.2` lanes COMPLETE; `.3` done: the census → decomposed; `.3.1` ADR-024; `.3.2` the rmcp pin; `.3.3` the read-half; `.3.4` the listen-stream durability) | repo-local |
