...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.6.1.5.4` — the policy LIBRARY takes site-operator authority, the last open child of `.6.1.5`: `POST /v1/policies` admits any enrolled principal over a first-come `(policy_id, version)` namespace. ⛔ `GET /v1/policies` and the resolve verb STAY on enrolment, and `.6.1.5.3`'s control asserts that. ✅ `.6.1.5.3.1` CLOSED (REPAIR-0282): seven published claims re-derived by independent routes — six hold, one was wrong (`reads 40 → 29` is **38 → 29**, a figure carried rather than measured), and the read census now names the documents that restate it. ✅ `.6.1.5.3` (0281); ✅ `.6.1.5.2.1` (0280); ✅ `.7.1.2.2.2` (0279); ✅ `.6.1.5.2` (0278); ✅ `.7.1.2.2.1` (0277). 🔎 OPEN: `.7.1.2.2.3` — a figure nobody can reconstruct | repo-local |
