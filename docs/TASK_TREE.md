...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.3.4.3.1.1.1` — 🔴 **A signed-off leaf closed on a premise a census has now refuted**: `.3.4.3.1.1` said *nothing enforces the stored value*, and three production sites do. ⭐ Its conclusion survives in one direction (a stored expiry later than the signed one is masked by the mTLS handshake) and is unexamined in the other — and the EARLIER direction is the one the enrollment path produced. ⛔ Historical, not live. ✅ **`.11.2.5` IS DONE (REPAIR-0248): the census gate was discharged by ANY command in the section, so every closed leaf discharged its own gap claims merely by having been verified** — `TASK-ACCEPTANCE` forces `cargo`/`make` into every `- [x]` box, and the gate measured **103 claim lines, 0 undischarged**: inert everywhere. ⭐ Six discharge tests priced against the same 103 (65.0% … **1.0%**), the 9.7% variant's extra nine classified and **6 found false**. **1 of 200 commits, 1 true positive, 0 false positives.** 🔴 Its first real catch was a false claim inside a signed-off leaf. ✅ `.11.18.2` (0247), `.11.18.1` (0246), `.11.20.2` (0245). | repo-local |
