...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.14.3.3` — the last child of the `.11.14.3` decision: the `claim_assessments` rows already written against invented identifiers, and the standalone `POST /v1/assessments` writer that still mints them. ⚠️ The honest disposition may be "nothing" — `.11.14.1` and `.11.14.2` both took the fail-closed no-backfill route — but it must be MEASURED and published, not assumed. ✅ **`.11.14.3.2` IS DONE (REPAIR-0218): a contribution's citation registers the §12.1 reference it names**, on the contribution's own transaction, so ROADMAP §13.2's step 2 and step 6 are both wired and the seam `.11.14.3` opened is closed. 🔴 Taking that decision found a second defect that made the obvious repair unshippable: `resources::submit` pre-checked on the LOCATOR alone and refused a differing digest, which on the contribution path becomes a **cross-tenant denial** — the first principal to pin a locator makes it uncitable by every other tenant, digest-less citations included — and whose 409 already leaked the cross-tenant existence §9.8 forbids. ⭐ The identity is the `(original_locator, expected_digest)` PAIR, which §12.1, §12.6, §9.8 and `migrations/0023:21` had all said before the code did; `locator_digest_conflict` is retired and `migrations/0065` makes the constraint `NULLS NOT DISTINCT`. Three residual findings are OWNED as `.11.14.3.4`–`.6`, not reported. ✅ **`.11.14.3.1` (REPAIR-0217)**: the `assess` step records an assessment against the thread. ✅ **`.11.14.3` (DOC-0037)**: the deliberation flow owns the evidence chain. | repo-local |
