...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.4.2` — handshake and lease fencing, where `lease_expires_at` is still received by the node and never read; `.4.1` is now closed end to end (the lapsed-token lockout, the unvalidated lifetime, and revocation's two halves) | repo-local |
