...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.9.1.2.3` — tranche 3c of the clause reconciliation. `.11.9.1.2.2` is closed by REPAIR-0178: 27 clauses, 22 `owned`, 3 `attach`, 1 `none`, 0 `unowned` — the ledger's FIRST `none` row, which completes the closed set. 🔴 It found the MCP read surface leaking across tenants on 3 of 3 tools while the module's own first paragraph claims the same authorization as HTTP: `policy_bundle` takes its tenant as an unused `_tenant_id` over a `SELECT` with no `WHERE`, `list_inbox` never reads its `principal`, and `get_thread` classifies and then returns the full projection anyway. `join_call` is the third instance of the two-caller-identifiers family. ⭐ `ATTACH-LANDED` has now fired on two consecutive tranches' unattached clauses before they were written | repo-local |
