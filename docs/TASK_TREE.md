...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.8.1` — the route census's key is wrong in BOTH directions: `stem()` deletes every `{param}`, so `/v1/snapshots/{id}` collapses onto `/v1/snapshots` and the ITEM route is counted *described* by the line documenting the SUBMIT route, while a mid-path parameter yields a stem no book ever writes and three documented routes read *absent*. `.11.8` published `35 described / 68 absent`; neither column stands. Opened by `.7.4.4`. ✅ **`.7.4.4` IS DONE (REPAIR-0254): one verb carried two acts that do not share an authority** — a snapshot two tenants cite is ONE row, and binding the delete verb to a *citing* tenant does not separate two citers, so A's delete stamped B's receipt with A's reason. `DELETE` now withdraws the caller's citation; tombstoning a shared row is an audited site act. ⛔ *Refuse when another cites* and *tombstone on the last withdrawal* both rejected — each leaks whether a stranger cites the row. ✅ **`.7.3.2.1` (REPAIR-0253)**: 1,896,248,619 bytes retained across 37 fixtures, 99.93 % reproducible payload; 34 reduced, 1,824,989,121 bytes dropped, **0 deleted**. ✅ `.7.2.8` (0252), `.7.2.9` (0251), `.7.2.10` (0250), `.7.2.7` (0249); `.7.2`'s mechanism table is fully discharged. ⚠️ Also opened: `.11.21` — `git.rs` is committed unformatted, because no per-commit check looks. | repo-local |
