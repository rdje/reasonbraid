...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.9.1` — 114 of 131 review records are cited by no candidate leaf, a backlog to CLASSIFY; `.4.2` and its seven children, `.4.1.4`, `.11.7` and `.4.2.8` are closed — `.4.2`'s clause-level reconciliation found SIX clauses its own split had dropped, and ALL SIX are now closed — the torn lease pair, a rotation that did not survive a restart, the wake gate's states, and `.4.1.5`'s three (a replacement restored the replaced machine's session, reverted its certificate's host, and never closed its incarnation) | repo-local |
