...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.18.2` — 🔴 **A calibration's 0 means nothing when the rule already polices the corpus.** Extending `GAP-CLAIM-CENSUS` to nested artifacts replays at 0 of 559 and that 0 is real; the IDENTICAL harness over the top-level corpus also returns 0 and that one is survivorship, because every commit that landed had been edited until it passed. ⚠️ The neighbour of `calibrate-over-the-history-that-contains-the-instance`, not the same trap — a WINDOW too short versus a POPULATION already enforced. ✅ **`.11.18.1` IS DONE (REPAIR-0246): the advisory and the blocker never governed the same corpus** — `--all` iterated 69 files (git's `*` crosses `/`) while the blocker filtered `^docs/tasks/[^/]*\.md$` and saw 15, so **102 claims across 6 files were advised and 97 across 5 enforced**, with 54 nested artifacts invisible. ⭐ The script spelled the population twice, once per path; one `governed_filter` now serves both, so the agreement is STRUCTURAL. ✅ The *dated record* defence was refuted by measurement (66 of 559 commits edit one) and the extension replays at **0 of 559**. ✅ `.11.20.2` (0245), `.11.20.1` (0244), `.11.17.2` (0243). | repo-local |
