...
| [`PHASE-8`](tasks/PHASE-8.md) | `active` | `.1.3` — the portable agent cards/profiles (opened at the Phase-7 close; `.1` done: the census → decomposed; `.1.1` done: ADR-026; `.1.2` done: the agreement machinery + the measured visibility widening) | repo-local |
