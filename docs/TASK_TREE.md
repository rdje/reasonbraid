...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.4.2.3` — a heartbeat can revive a lease that expired between the check and the write; `.4.2.1`/`.4.2.6`/`.4.2.2` closed the rotation ordering, the proof ladder and proof replay | repo-local |
