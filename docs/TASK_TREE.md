...
| [`PHASE-8`](tasks/PHASE-8.md) | `active` | `.5` — the regional routing lane (opened at the Phase-7 close; **the `.1`–`.4` lanes COMPLETE** — the federation, the A2A facade, the MCP surface, the adapter/resolver SDK) | repo-local |
