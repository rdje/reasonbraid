...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.26` — one of `COMMIT.md`'s four pre-push gates failed once in fifteen and its evidence was discarded by the `tail -4` it was run through. ✅ **`.11.27` (REPAIR-0316): a self-test that pins one host's collation is not ground truth** — the first push of this session turned `doctrines` RED on the runner while green locally, because an extractor's `sort -u` ordered by the ambient locale; reproduced with the shipped bytes under `LC_ALL=C`, and the INSTRUMENT is pinned rather than the comparison loosened. ✅ **`.11.24.1.2` (0315): §10.2's sixth presence state is reachable.** ✅ **`.11.25.1.1` (0314): a censored observation is not a duration.** ✅ **`.11.25` (0313): the wait I wanted to shorten is the detector** | repo-local |
