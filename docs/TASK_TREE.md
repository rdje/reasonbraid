...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.6.1.5.2` — store the tenant the lifecycle write already derives: the migration adding `tenant_id` to the nine lifecycle tables plus the 15 write sites. ⛔ NOT the reads — that split exists so no row is hidden from its own author. ✅ `.6.1.5.1.1` CLOSED (REPAIR-0276): **two gates were open AND unobservable** — a foreign tenant registered a proposal against another tenant's thread, and recorded a governance DECISION on another tenant's proposal. The census answers the systemic question: 3 of 5 claim sites already carry an explicit predicate, so the two lifecycle verbs were outliers. ✅ `.6.1.5.1` CLOSED (REPAIR-0275); ✅ `.6.1.5` CLOSED (DOC-0071) | repo-local |
