...
| [`PHASE-8`](tasks/PHASE-8.md) | `active` | `.4` — the adapter/resolver SDK (opened at the Phase-7 close; **the `.1`/`.2`/`.3` lanes COMPLETE** — the `.3` MCP surface: ADR-024 + the rmcp pin + the read/write tools + the listen-stream durability + the qualified gate + the live tool-path roundtrip) | repo-local |
