...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.7.2.8` — three ref-verification clauses owned by the `.7.2` lane and carried by no child, still undischarged after `.7.2.7` and `.7.2.9` closed the other two mechanisms that table named. ✅ **`.7.2.9` IS DONE (REPAIR-0251): gix ships a 16 MiB per-object allocation brake and this project had it switched OFF by a property that is otherwise a virtue** — the default applies only at `Trust::Reduced` and `init_opts` hardcodes `Trust::Full`, so every acquisition repository, fully trusted BECAUSE the server created it, let a remote-supplied pack entry declare any size. ⭐ The defect lives in the seam between two correct decisions in two codebases. ✅ The limit is now `max_bytes`, the ceiling already declared, so it refuses nothing that ceiling allowed. ✅ `.7.2.10` (0250), `.7.2.7` (0249), `.3.4.3.1.1.1` (DOC-0047). | repo-local |
