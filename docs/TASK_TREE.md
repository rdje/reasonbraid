...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.7.3.2.1` — the browser suite's retained fixtures accumulate and nothing retires them, opened by `.7.3.5` during its own §8 cleanup. ⭐ `.7.2`'s mechanism table is now fully discharged. ✅ **`.7.2.8` IS DONE (REPAIR-0252): three ref clauses, and a FOURTH defect that made all three unreachable** — `acquire_into` passed the URL's fragment to `remote_at`, so every selector-bearing acquisition failed at the remote, and it survived because **nothing ever drove the feature** (13 call sites, none with a fragment). 🔎 A reading found three real defects in unreachable code; the first control found why. ✅ An annotated tag resolves via `Ref::Peeled`; an ambiguous short name is refused by name listing both refs, only when the commits differ; `refs/heads/..` is refused. ✅ `.7.2.9` (0251), `.7.2.10` (0250), `.7.2.7` (0249). | repo-local |
