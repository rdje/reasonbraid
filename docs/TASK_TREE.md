...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.14.3.11` — `POST /v1/snapshots` names a `reference_id` in its BODY and never checks the caller may use it; ⭐ a gap in `.11.14.3.4`'s own census, which enumerated the routes under `/v1/resources`. ⛔ Do NOT assume symmetry with the read settles it: a submission carries the BYTES, which is the asymmetry that made the snapshot replay safe and cuts the other way here. ✅ **`.11.14.3.12` IS DONE (REPAIR-0225)** — and the finding was that the disposition was **already decided**: `.7.4.2` shipped `acquisition_error { kind: "evidence_unstored" }` on the R2 arm with the reason written at the call site, and the R0 and R5 arms were never migrated onto it. 🔴 RED returned a complete acquisition receipt — digest, byte count, redirect chain — with **0** snapshots and no error at all. No new field, no wire change. ✅ **`.11.14.3.6`** (REPAIR-0224); ✅ **`.11.14.3.4`** (REPAIR-0223); ✅ **`.11.14.3.8`** (REPAIR-0222); ✅ **`.11.14.3.3`** (REPAIR-0219); ✅ **`.11.14.3.2`** (REPAIR-0218). | repo-local |
