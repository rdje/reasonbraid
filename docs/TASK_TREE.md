...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.6.2.2` — **the MCP listen cursor is not monotonic**: recording 100 then 5 leaves it at 5, and the resume path reads that as the OWN cursor, so one out-of-order delivery rewinds the resume point. Decide clamp vs refusal with a reason — they differ observably. ✅ `.6.2.1` CLOSED (REPAIR-0288): the dedup window kept the OLDEST 64 and now keeps the newest; the repair is a direction and the control crosses the boundary the old two-delivery one never reached. ⚠️ `.6.2.3` (a malformed window is silently emptied) and `.6.2.4` (the transport BUILD, ranked last) remain. ✅ `.6.1` is FULLY CLOSED | repo-local |
