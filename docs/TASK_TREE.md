...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.14.2` — `GET /v1/claims/{claim_id}/assessments` is enrolment-gated over a namespace the server never mints: `git grep -n "CREATE TABLE claims" -- migrations` returns **zero**, `claim_id` is caller-supplied `TEXT`, and the shipped control's own id is `clm_budget` — an ORACLE over guessable ids, while its sibling over the SAME table is bound. ✅ **`.7.4.3` IS DONE (REPAIR-0214): the retention sweep is a site-operator act on the server's clock.** RED was HTTP 200 and `{"tombstoned":2}` — one enrolled principal naming the year 3000 tombstoned BOTH tenants' live rows. It now takes the new `evidence_expire` site capability, audited like every other site act, and the caller's `at` is gone from the wire rather than bounded: a deletion stamped "the retention expired" is a factual claim, so the cutoff is the database's own post-lock `clock_timestamp()`. ⛔ It cannot be a tenant verb — `retention_class` is a column on the SHARED row, so which rows are due is a site fact no citation owns. 🔎 It also found and fixed a defect in REPAIR-0213's own control: an enrolment-gate leg asserted 401 while presenting a malformed id, so it measured the header parser; the gate answers 403, and the book said 401. ✅ **`.11.14.1` IS DONE (REPAIR-0213): the evidence reads are bound to the CITING TENANT.** The derivation path was measured and every candidate failed — `submitted_by` is a one-way `Uuid::new_v5` that joins to no identity table, and `resources::submit` replays on the LOCATOR ALONE so it names the FIRST citer whatever happens after — so the binding is the join the schema lacked: `evidence_citations (snapshot_id, tenant_id)`, written on the fresh insert AND the replay. A shared row carries BOTH citers, which is why a column would have hidden it from one of its own authors. ⛔ `stale` stays a TENANT read, on a census: nothing operator-shaped consumes it. | repo-local |
