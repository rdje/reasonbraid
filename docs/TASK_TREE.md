...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.7` — the published reason-code registry and the codes the product emits have drifted; `.4.2`'s seven children and `.4.1.4` are closed, and `.11.9`/`.11.4.2.1` delivered two tracked census instruments and rejected both gates they proposed | repo-local |
