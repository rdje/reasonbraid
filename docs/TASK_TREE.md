...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.14.3.7` — `thread.contribute` carries **no** quota and no length bound, so `.11.14.3.2`'s citation registration turned unbounded input into O(n) statements inside the transaction holding the thread's `FOR UPDATE` row. ⛔ A cap was deliberately not invented: §16.11's machinery exists and reaches one verb; which verbs and which dimension is the decision. ✅ **`.11.14.3.5` IS DONE (REPAIR-0228) — with ONE repair and TWO stated contracts, which is the honest count.** 🔴 **`scheme` is REFUTED**: I read a test helper's comment as a missing check, wrote it, reproduced RED, went GREEN — and **two of the suite's own controls refused it**. Measured instead of inferred, the consumer is `resolvers::resolve`'s `WHERE schemes @> …` and TWO SHIPPED packs pair a non-URI scheme with an `https://*` locator pattern (`git`, `web+render`): the field asks for a CAPABILITY. No check; the contract is stated at the type and in the book, a control arm pins the refutation, and the misleading comment is corrected at its source. 🔎 The refusing controls exercise the PACKS — **a repair is falsified by the code that USES the thing** — and running the affected suites BROADLY is what surfaced it. 🔴 The real defect was the dead column defaults: `#[serde(default)]` was the EMPTY STRING, so the two writers disagreed for the same omission; they now agree. ⭐ The fragment stays in the locator, the same boundary `.11.14.3.13` drew from the other side. ✅ **`.11.14.3.13`** (REPAIR-0227); ✅ **`.11.14.3.11`** (REPAIR-0226); ✅ **`.11.14.3.12`** (REPAIR-0225); ✅ **`.11.14.3.6`** (REPAIR-0224); ✅ **`.11.14.3.4`** (REPAIR-0223); ✅ **`.11.14.3.8`** (REPAIR-0222). | repo-local |
