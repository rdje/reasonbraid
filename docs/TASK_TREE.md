...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.9.1` — 114 of 131 review records are cited by no candidate leaf, a backlog to CLASSIFY; `.4.2`'s seven children, `.4.1.4` and `.11.7` are closed, and three tracked census instruments now exist — two whose proposed gates the measurement rejected, one (`REASON-CODE-DOC`) it justified | repo-local |
