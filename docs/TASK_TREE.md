...
| [`PHASE-7`](tasks/PHASE-7.md) | `active` | `.4.1` — the load harness (the `.1`/`.2` lanes COMPLETE; `.3` done: the extraction criteria; `.4` done: the census → decomposed `.4.1` harness → `.4.2` runbooks → `.4.3` game-days) | repo-local |
