...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.7.3.6.5` — **the terms with no stated meaning, defined where a caller reads them or corrected**: 7 `undefined` (`archive_policy` on all six packs, defined nowhere; R3's `allow-bounded`) and 7 `misdescribed` (`listed` on five packs that consult no list; RX's `any` on a pack that performs no egress; **R3's `vm_container`, which is G4's open strand**). ✅ **`.7.3.6.4` (REPAIR-0269): one test served two ladders running in opposite safety directions** — the egress filter refused 1 combination of 24 and refused the wrong one; the code moved, ADR-018 stands. ✅ **`.7.3.6.3` (REPAIR-0268)**: the pair was gated at cadences ~300 commits apart; the decision is now a pure function. ✅ **`.7.3.6.2` (REPAIR-0267)**: `register` INSERTed 18 columns and updated 6. ✅ **`.7.3.6.1` (REPAIR-0266): 3 of the 36 advertised policy lines are enforced** | repo-local |
