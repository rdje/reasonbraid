...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.4.2.5` — the key-storage claim is wider than the code, the LAST unclosed child of `.4.2`; `.4.2.1`–`.4.2.4`, `.4.2.6`, `.4.2.7` closed the rotation ordering, proof replay, the proof ladder, both check-vs-commit windows and the panicking hex decoder | repo-local |
