...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.26` — **one of `COMMIT.md`'s four pre-push gates failed once in fifteen** and its evidence was discarded by the `tail -4` it was run through; 14 consecutive greens since, including a dedicated ten-run loop. ⛔ Not dismissed as a flake — the gate is the condition for spending a CI run on a public remote. ✅ **`.11.25.1.1` (REPAIR-0314): a censored observation is not a duration, and a blind instrument's silence is not an absence** — a published figure corrected and a second finding refused before publication. ✅ **`.11.25` (0313): the wait I wanted to shorten is the detector.** ✅ **`.11.24.1.3.1` (0312): a render is evidence.** ✅ **`.11.24.1.3.1.1` (0311): the render's parent was its own derivation** | repo-local |
