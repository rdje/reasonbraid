...
| [`PHASE-2`](tasks/PHASE-2.md) | `active` | `.1.3` — revocation surfaces (`.1.2` complete: `.1.2.1` cert issuance at enrollment + `.1.2.2` the channel v3 cert-proof handshake + rotation; then `.1.4` delegation → `.1.5` caching → `.1.6` incarnations) | repo-local |
