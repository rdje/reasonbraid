...
| [`PROGRAM`](tasks/PROGRAM.md) | `active` | index only — next executable work is `PHASE-7.1` | repo-local |
...
| [`PHASE-8`](tasks/PHASE-8.md) | `active` | `.3.5.3` — the live tool-path roundtrip (opened at the Phase-7 close; the `.1`/`.2` lanes COMPLETE; `.3` done: the census → decomposed; `.3.1` ADR-024; `.3.2` the rmcp pin; `.3.3` the read-half; `.3.4` the listen-stream durability; `.3.5` the write-half census → decomposed `.3.5.1`–`.3.5.3`; `.3.5.1` the qualified gate + the quota binding; `.3.5.2` the three write tools) | repo-local |
