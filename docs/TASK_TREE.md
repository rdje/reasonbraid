...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.3.4.2` — the replay hash does not bind the authority context, so two requests differing only in `on_behalf_of` share an idempotency key; `.3.4.1` closed by refusing its own premise — `delegable` governs grant chains that have no producer | repo-local |
