...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.7.3.6.2` — **the registry's upsert writes 6 of its 18 columns**, so a corrected advertisement never reaches an existing row, and `media_types` — the field `docs/decisions/2026-09-12_r2-acquisition-accept-set.md` promises a re-registration narrows *in the same act* — is one of the 11 it drops. ✅ **`.7.3.6.1` (REPAIR-0266): 3 of the 36 advertised policy lines are enforced** with a RED on record — `enforced` 3 · `unverified` 5 · `vacuous` 14 · `misdescribed` 7 · `undefined` 7. 🔴 Four of the six policy fields are consumed by NOTHING (34 field mentions on 32 lines, 0 reads); `archive_policy` is defined nowhere in the repository. 🔴 The census carried the defect it was built to find until falsification put the advertised VALUE into the verdict's key. `.7.3.6` is now a parent: `.2` the upsert, `.3` the advertisement-versus-enforcement pair, `.4` the egress ladder's direction, `.5` the undefined and misdescribed terms | repo-local |
