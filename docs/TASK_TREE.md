...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.3.5.1` — the global one-unused-token index; `.11.4.3.1.7.2` made the enforcer run every instrument's own `--self-test`, and the gate caught itself recursing while being built | repo-local |
