...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.2.5` — 🔴 **`GAP-CLAIM-CENSUS` is discharged by ANY enumerating command in the claim's section, including one about something else** — the gate's own DECLARED LIMIT (*it verifies the census was RECORDED, not that it was RUN*), with a `.11.18` fixture pinning it. ⚠️ The leaf owns whether the limit is closable at all, or whether the honest move is to state it where readers meet it. ✅ **`.11.18.2` IS DONE (REPAIR-0247): a replay over policed history measures DETERRENCE, not the rule's cost, and reports it in the same units** — two calibrations returned 0 of 559 and only one meant anything. ⭐ ABSORBED into the window note, which needed a third clause beyond `.11.20.2`'s criterion: same activity, different failure mode, decided by the host already claiming the reader's question — **a note that advertises a question owes every answer to it**. ✅ Census: **2 of 3** calibrating instruments replay over a corpus their own `--check` gates; detection costs **17 ms**; the distortion grows to 100% exactly when a mature gate is re-argued. 🔴 Adding the arms caught a banner printing `23 arms` over **21**. ✅ `.11.18.1` (0246), `.11.20.2` (0245), `.11.20.1` (0244). | repo-local |
