...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.3.4.3.1.3` — the certificate's opposite skew direction; `.3.4.3.1.2` closed the dispatch outage by letting the node tell the time in the server's terms, superseding `.3.4.3`'s clamp | repo-local |
