...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.24.1.3.1` — **the R3 render persists nothing**, no snapshot and no edge, although §12.6 opens on exactly that artefact; the blocker is one wire field (`BrowseWorkerResponse` carries `parent_digest` and not the rendered bytes, while a snapshot's `raw_digest` is a foreign key into `snapshot_objects (bytes BYTEA NOT NULL)`). ✅ **`.11.24.1.3` (REPAIR-0310): the missing derivation edges are a SYMPTOM** — GIT and BROWSE record NOTHING, and one cause covers both: the evidence store can only hold an artefact that is a byte string the process has in memory. ✅ **`.11.24.1.1.1.1` (0309): a receipt is earned by an offer** — the ack's *withheld now* proxy over-recorded, so a node could make undelivered work deletable by acking past rows it was never offered. ✅ **`.11.24.1.1.1` (0308): the tail read IS the offer.** ✅ **`.11.24.1.1.2.2` (0307): a proof of allowance must carry its own window** | repo-local |
