...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.7.3.6.3` — **the advertisement and the enforcement are two constants a comment holds together**: `--readers` returns 0 reads across all four `*_policy` fields, so flipping the advertised word changes what callers are told and not what the browser does. ✅ **`.7.3.6.2` (REPAIR-0267): `register` INSERTed 18 columns and updated 6.** Reproduced both halves; the acceptance's own re-read found `.11.9.1.1.1`'s finding that the table has NO tenant column, so writing all 18 would have handed an unbound principal eleven more columns on a site-global row — rejected. Split by caller: the boot sync replaces completely, the HTTP verb refuses by name. **58 passed, 0 failed**. ✅ **`.7.3.6.1` (REPAIR-0266): 3 of the 36 advertised policy lines are enforced** — `enforced` 3 · `unverified` 5 · `vacuous` 14 · `misdescribed` 7 · `undefined` 7; four of the six policy fields are consumed by NOTHING. Remaining: `.4` the egress ladder's direction, `.5` the undefined and misdescribed terms (**G4's open strand**) | repo-local |
