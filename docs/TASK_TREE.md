...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.2.6` — **the acceptance gate reads ONE checklist per staged tree file (this tree has 194) and hard-gates a vocabulary the project does not use** (`NO REGRESSION` 203 · `REPRODUCE / ISSUE` 142 · `FIX / LOCKSTEP` 192, against its own `ADDRESSED` 14 · `ROOT CAUSE` 13). ✅ Calibration done: four candidates priced over 300 commits, **deepest-closed-leaf + corpus-derived vocabulary at 20.9%**, refusals verified as true positives; implementation is next. ✅ **`.3.5.5` (REPAIR-0261)**: `GET /v1/nodes/presence` asked nobody who was calling — reproduced at 200 against 401, repaired by authenticating and deriving the tenant, foreign node answers `unknown_node` as an absent one does. ✅ **`.3.5.4` (REPAIR-0260)**: the read census could not see 34 of its own routes and its population had no producer — 24 published, **58** real; all 58 now classified by `scripts/census_get_route_binding.py`. | repo-local |
