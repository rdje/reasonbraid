...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.17` — a published `file:line` is a bare basename and **29 of them name two files**, and the positions drift: `.11.18` re-measured two of its own pinned lines from `:4510`/`:4514` to `:4573`/`:4577` inside one session. §4.1 grades a NAMED INSTANCE as exact with no tolerance band. ✅ **`.11.18` IS DONE (REPAIR-0235): `GAP-CLAIM-CENSUS`'s section scanner did not know what Markdown is** — it called every `^#{1,6} ` line a heading, so a shell comment inside a fenced CENSUS opened a pseudo-section and the claim above it lost the census three lines below, which is exactly how it BLOCKED `.11.14.3.2`. 🔴 **The leaf's own claim about the defect's DIRECTION was REFUTED**: a pseudo-section starts later than and ends no later than its real section, so it is a strict SUBSET and can only WITHHOLD a discharge — false positives only — and the fixture built for the asserted silent direction classifies identically before and after (pinned as the gate's declared limit). ⭐ A second instance of the same defect fixed with it (an indented ATX heading; measured 82 → 82, 0 newly blocked) and ⚠️ an unbalanced fence now REFUSES rather than guessing. Population unchanged: 94 claims, 0 unbacked. Promoted as `a-scoping-defect-errs-in-one-direction`. ✅ **`.11.14.3.10`** (REPAIR-0234, closing the `.11.14.3` family); ✅ **`.11.14.1.2`** (REPAIR-0233); ✅ **`.11.14.1.1`** (REPAIR-0232). | repo-local |
