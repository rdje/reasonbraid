...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.3.4.3.1.2` — let the node evaluate server instants in the server's terms; `.11.4.3.1.7.1` repaired a self-test that searched for a string it contained, unblocking the §8 review that freed 1.59 GB | repo-local |
