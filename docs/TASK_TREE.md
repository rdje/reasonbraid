...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.6.1.5.2.1` — the five lifecycle writes that act on a publication nobody checks is theirs (`stage_publication`, `record_policy_drift`, `record_policy_correction`, `record_policy_outcome`, `schedule_policy_reviews`, each admitting on `reader_tenant(…).is_some()`); then `.6.1.5.3` (the 32 reads, pinned by name in the leaf). ✅ `.7.1.2.2.2` CLOSED (REPAIR-0279): a suite purges what it asserts over AND what it writes — only the second is mechanizable, and the new `census_fixture_write_reach.py` gate swept 5 plans. ✅ `.6.1.5.2` CLOSED (REPAIR-0278): the nine lifecycle tables store the tenant the write already derived — four from the caller, five from their PARENT. ✅ `.7.1.2.2.1` CLOSED (REPAIR-0277): the fixture-plan gate could not see `ALTER TABLE … ADD COLUMN … REFERENCES` | repo-local |
