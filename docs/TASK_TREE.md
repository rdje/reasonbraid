...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.9.3.1` — citing an authority is treated as holding one: `authority_holds` takes the grant id and nothing else, behind an enrolment-only endpoint, on two surfaces. 🔴 `.6.1.1` is closed by REPAIR-0182, the severest defect this reconciliation produced: the three MCP read tools ran a PRIVATE re-implementation of an authorization their own module header said they shared, and a live control measured a caller in one tenant receiving another tenant's entire thread projection. The three reads now CALL the HTTP handlers' own gates through the new `mcp_read_internal` seam; the copy is deleted. ⚠️ **One part of the finding was corrected by measurement rather than confirmed**: the policy registry has no tenant column and no site filters by one, so the bundle was never crossing a tenant boundary — it is site-wide on both surfaces, and the tool's real debts were the enrolment gate and a false tenant label. The schema question is opened as `.6.1.5`. Rows 1b–1d are `.6.1.2`, `.11.2.3`, `.9.2.1`; tranche 4b is row 2 | repo-local |
