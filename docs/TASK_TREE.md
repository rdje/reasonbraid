...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.6.1.3` — the MCP write seam indexes an UNTYPED body: `body["tenant_id"] = …` on a `serde_json::Value` received untyped, and `IndexMut<&str>` PANICS on a scalar or an array. ⚠️ Not reachable through the typed MCP tool today; the guard belongs at the `pub` seam. ✅ `.7.1.2.2.3` CLOSED (REPAIR-0284): the figure `.6.1.5.3.1` could not reproduce is exactly right — it is the distinct site-global tables over the `identity only` + `walk` rows — and the real defect was that `residue` was measured against ONE decision record, which ages the moment a second is written; now 17 tables, residue 0, both printed by `--check`. ✅ `.6.1.5.4` (0283); ✅ `.6.1.5.3.1` (0282); ✅ `.6.1.5.3` (0281); ✅ `.6.1.5.2.1` (0280) | repo-local |
