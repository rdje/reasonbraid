...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.7.1.2` — **adjudicate the 33 site-global writes admitted by enrolment alone**, the population `.7.1.1` measured and deliberately did not judge (30 tables; `policy_*` 10, `evaluation_*` 7). ⚠️ A population, not 33 defects. ✅ **`.7.1.1` (REPAIR-0271): the population is 42, not 2** — 72 mutating routes, 42 writing a table with no tenant dimension, 33 of them on enrolment alone, and `.3.2`'s closed set of six site actions covers none. 🔴 Three instrument defects found on the way, two in shipped code (a schema-qualified `CREATE TABLE` invisible to the shared tenant-dimension primitive; `UPDATE` in three non-write SQL positions reported as tables). ✅ `.7.3.6` closed earlier with all five children (REPAIR-0266–0270) | repo-local |
