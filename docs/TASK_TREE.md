...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.6.1.4` — the quota's admission semantics, and the control that does not test them. ⚠️ The measured position is NOT a defect and is recorded so the leaf cannot manufacture one: `gate()` commits its quota use in its own transaction before idempotency applies, and the seam's header says exactly that. What IS open is a test heading naming three things as one pipeline that the seam deliberately splits across two transactions, with no control observing the split. ✅ `.6.1.3` CLOSED (REPAIR-0285): the write seam refuses the body it cannot index — observed RED as a serde_json panic — and the repair is a CHECK rather than a retype because the body feeds an idempotency hash. ✅ `.7.1.2.2.3` (0284); ✅ `.6.1.5.4` (0283); ✅ `.6.1.5.3.1` (0282); ✅ `.6.1.5.3` (0281) | repo-local |
