...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.24.1.1.2.1.1.1.1` — **`federation_agreements` records `proposed_at` and `accepted_at` and leaves the revocation undated beside them.** ⭐ A sharper instance than the one that found it: `authority_grants` recorded no transition instants at all, so its omission was uniform; here the missing one is a gap in a pattern the schema itself establishes. ⚠️ NOT a claim anything needs it yet — `.11.6` says census the READERS first. ✅ **`.11.24.1.1.2.1.1.1` (REPAIR-0303): revoking an authority records when**, written by the SAME statement that changes the status, valued with the database time that also stamps the effect record — so the row and the audit trail agree by construction. ✅ **`.11.24.1.1.2.1.1` (0302): an expired row prunes on its own window, a revoked one now on the revocation instant.** ⭐ A retention window measures time in the STATE being retained. ✅ **`.11.24.1.1.2.1` (0301): a cursor acknowledgement stops receipting rows the tail withheld.** ✅ **`.11.24.1.1.2` (0300): a command cannot outlive the authority that admitted it**, and ⛔ the max age is DERIVED, never chosen. 🔴 **`.11.24.1.1` (0297): `delivery_state` published `acknowledged` for §10.6's `transport_received`**, the one state the suite never asserted | repo-local |
