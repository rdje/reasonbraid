...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.18.1` — 🔴 **`GAP-CLAIM-CENSUS`'s blocking path filters `^docs/tasks/[^/]*\.md$`, so every task document in a SUBDIRECTORY is invisible to it**, and `.11.18`'s own population census found half its instances there. ⚠️ The leaf owns DECIDING the governed population rather than assuming it; afterwards `--all` and the staged path must enumerate the same corpus or the difference is stated in the script. ✅ **`.11.20.2` IS DONE (REPAIR-0245): the standing lesson is a note, and the extraction criterion is the deliverable** — *absorb when the host note's THESIS is your rule; extract when your rule is a TOOL it merely uses*, which is the named difference from `.11.17.2`'s opposite ruling one commit earlier. ⭐ New beyond relocation: a NEGATIVE arm cannot be falsified against the old code and needs a DEGENERATE implementation of the new rule. 🔴 The acceptance's own last clause was unsatisfiable — a precedence chain let a leaf citation SHADOW the method anchor — now repaired to report both. ✅ `.11.20.1` (0244), `.11.17.2` (0243), `.11.20` (0242). | repo-local |
