...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.7.1.2` — **re-scoped from 30 tables to 17**: `.11.14` is `done` and DOC-0029 already decides the site-global data model (evidence content-addressed by design, no table tenant-owned, the READ tenant-bound), which `.7.1.1` published a population without checking. ⭐ The difference is a number — `.11.14` decided TWELVE, a census from the WRITE side finds THIRTY, and **17 are unnamed in the record** (nine of ten `policy_*`, both `routing_*`, `workflow_profiles`, …). ⛔ Cause: `.11.14`'s census was scoped by FAMILY NAME, not derived from the producers. ✅ `.7.1.1` (REPAIR-0271) published the 42/33 population and repaired three instruments. ✅ `.7.3.6` closed with all five children (REPAIR-0266–0270) | repo-local |
