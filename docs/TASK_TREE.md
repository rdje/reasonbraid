...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.3.3.4.12` — federation direction administration: the three direction verbs take no guard at all, which is also where the card import's ordering against agreement revocation arrives | repo-local |
