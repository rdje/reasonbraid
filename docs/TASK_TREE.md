...
| [`PHASE-8`](tasks/PHASE-8.md) | `active` | `.3.1` — the census + ADR-024 (opened at the Phase-7 close; the `.1`/`.2` lanes COMPLETE; `.3` done: the MCP census → decomposed `.3.1` ADR-024 → `.3.2` the pin → `.3.3` the read-half → `.3.4` the listen-stream durability → `.3.5` the write-half profile) | repo-local |
