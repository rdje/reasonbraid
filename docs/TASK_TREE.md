...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.2.4` — the visibility gate enumerates four sentence shapes anchored on the full word `repositor(y\|ies)`, so two live instances of the superseded private-repository claim sit in the corpus invisibly as `the repo is private`. ⛔ Row 1 briefly named `.7.4.5`, `done` since REPAIR-0216: a frontier pointer at a closed leaf, which `INDEX-FRONTIER` cannot catch because it compares the two files to each other, not either to its leaf's own Status. ✅ **`.11.22` IS DONE (REPAIR-0257): the frontier table gained a row per state change and never retired the old one** — 13 leaves with 2–4 rows, 10 status columns contradicting their leaf, including one pair I wrote two commits earlier; censused before the rule, 9 stale rows removed and `FRONTIER-STATUS` registered at 0.06 s. ✅ **`.11.21` (REPAIR-0256): unformatted Rust reached `main` and stayed, because nothing looked per commit** — a policy gap, since the enforcer had no formatting check and `cargo fmt --check` sat only in the pre-push list with pushes batched at ~300 commits; measured at 0.66 s against a 12.6 s enforcer and registered as `RUST-FORMATTING`, seen RED against injected unformatted code. ✅ **`.11.8.1` (REPAIR-0255)**: a route key too loose and too TIGHT in one expression; 21 of 104 routes misclassified, corrected to 49/3/52 and `.11.8`'s `22/1/80` superseded by 30/3/70 at its own commit. ✅ **`.7.4.4` (REPAIR-0254)**: `DELETE /v1/snapshots/{id}` withdraws the caller's citation; tombstoning a shared row is an audited site act. ✅ **`.7.3.2.1` (REPAIR-0253)**: 1.9 GB retained, 99.93 % reproducible payload, 34 fixtures reduced and **0 deleted**. ✅ `.7.2.7`–`.7.2.10` (0249–0252). | repo-local |
