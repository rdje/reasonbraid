...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.19.1` — 🔴 **twelve verification-log rows in `docs/tasks/PHASE-2.md` lost their first two cells**, the date and the leaf id, and the renderer shows the damage is not uniform: one falls OUT of the table into a paragraph, the others render with their columns shifted left and padded by empty cells. 🔎 **Both table gates are blind to the shape** — `check_table_arity.sh --all` reports `0` because a pipe-less row has no header to disagree with, and `BROKEN-TABLE`'s scanner stops at the first pipe-less line. Two instruments, one shared model of what a row is, and the defect lives in the gap. ✅ **`.11.19` IS DONE (REPAIR-0238): a blank line ENDS a GFM table**, and one sat inside this tree's OWN Current Frontier, orphaning 46 rows including row 1 — the cell `INDEX-FRONTIER` and `BOOK-FRONTIER` both exist to police. **5 blanks across 3 files, 52 rows, discharged to 0** and verified by the RENDERER rather than by reading the source (`<tr><td>` 157→203, 53→56, 29→32 — **+52 exactly**, matching the census by a completely different route). ⭐ **`BROKEN-TABLE` SHIPS**, calibrated across 200 commits before it was proposed: **1 commit blocked (0.5%)**, and that commit is the one that INTRODUCED the defect — zero false positives, `REASON-CODE-DOC`'s shape. ⭐ Every parsing rule is the RENDERER's answer, which corrected two the ad-hoc probe had wrong. 🔴 And the leaf published a **0** that measured **12**; the superseded claim is kept, because checking it is what found `.11.19.1`. ✅ `.11.16` (REPAIR-0237). | repo-local |
