...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.7.4.4` — a snapshot two tenants cite is ONE row, so either can tombstone it for both, irreversibly; it needs CO-CITATION to reach the verb and opens from a derived candidate rather than a blank page. ✅ **`.7.3.2.1` IS DONE (REPAIR-0253): a retention rule with no retirement, on both populations at once** — 1,896,248,619 bytes across 37 fixtures, **99.93 % of it reproducible payload**, and `target/pg-tests` had more than doubled while the leaf sat pending. ⭐ What a fixture must keep was measured, not chosen: the worker writes `owner.json`/`completion.json` INSIDE the payload directory, so the obvious rule would have destroyed exactly what `.7.3.2` preserves. 34 of 37 reduced, 1,824,989,121 bytes dropped, **0 deleted**. 🔴 Mutation found three of the instrument's own controls vacuous, the EPERM branch among them. ✅ `.7.2.8` (0252), `.7.2.9` (0251), `.7.2.10` (0250), `.7.2.7` (0249); `.7.2`'s mechanism table is fully discharged. | repo-local |
