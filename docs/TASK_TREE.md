...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.9.1` — 114 of 131 review records are cited by no candidate leaf, a backlog to CLASSIFY; `.4.2` and its seven children, `.4.1.4` and `.11.7` are closed — and `.4.2`'s clause-level reconciliation found SIX clauses its own split had dropped, now owned by `.4.2.8`–`.4.2.10` and `.4.1.5` | repo-local |
