...
| [`PHASE-1`](tasks/PHASE-1.md) | `active` | `.1.8.1` — the audit-reconstruction demo leg (`.1.8` decomposed at the census seams; then `.1.8.2` the G1–G2 gate package + Phase 1 close; `.1.7` complete: `.1.7.1` `make dev` + `.1.7.2` release packaging + LAN runbook; `.1.6` complete: budget read + embedded shell + demo beat; `MAINT-1`…`MAINT-3` closed) | repo-local |
