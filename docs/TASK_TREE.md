...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.7.1.2.2.3` — a published figure nobody can reconstruct fails the durability leg whatever its truth: `.7.1.2.2`'s `29 tables → 27` matches no quantity either baseline pins, and `residue 16 → 14` needs the same pass. ✅ `.6.1.5.4` CLOSED (REPAIR-0283) and `.6.1.5` with it: the governance LIBRARY takes `policy_register` site authority — the takeover reproduced at runtime first (Mallory took `red-org-baseline 1.0.0`; Alice's own registration at that coordinate was refused as a duplicate and she then read Mallory's clause as the baseline), the READS asserted unchanged, and the control caught two further defects inside the repair itself. ✅ `.6.1.5.3.1` (0282); ✅ `.6.1.5.3` (0281); ✅ `.6.1.5.2.1` (0280); ✅ `.7.1.2.2.2` (0279); ✅ `.6.1.5.2` (0278) | repo-local |
