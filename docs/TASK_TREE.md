...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.7.1.2.1` — 🔴 **a LIVE cross-tenant control defect**: `workflows::resolve` takes the highest version of a `profile_id` site-wide, `workflows::register` appends `MAX(version)+1` for ANY id including the eight built-ins, and `POST /v1/workflow-profiles` admits on **enrolment alone** — so any enrolled principal can re-register `quick_advice` and change the steps every other tenant's next bare thread runs. ⚠️ SOURCE-measured at four sites; the runtime RED is owed FIRST. ✅ `.7.1.2` CLOSED (DOC-0066): the seventeen adjudicated **4 · 1 · 2 · 10** — four were never ownerless (they key into a tenant-dimensioned table), the evaluation family is SEVEN not six, and **ten are shared CONTROL surfaces** whose problem is the WRITE, so DOC-0029's tenant-bound-read remedy reaches none of them. ⛔ `.3.2`'s six site actions upheld as actions, SUPERSEDED as THE set. `.6.1.5` re-scoped from one table to ten by name | repo-local |
