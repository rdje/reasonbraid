...
| [`SIGNOFF-REPAIR`](tasks/SIGNOFF-REPAIR.md) | `active` | `.11.21` — `git.rs` is committed UNFORMATTED, and the cause is a policy gap: the per-commit enforcer has no formatting check and `cargo fmt --check` lives only in the pre-push list, so unformatted Rust can sit on `main` for as many commits as pass between pushes. ✅ **`.11.8.1` IS DONE (REPAIR-0255): a route key too loose and too TIGHT in one expression** — deleting path parameters collapsed `/v1/snapshots/{id}` onto `/v1/snapshots` (the item credited to the submit route's line) and made a mid-path parameter match nothing; 21 of 104 routes misclassified, corrected to **49 described / 3 mentioned / 52 absent**, and `.11.8`'s published `22/1/80` superseded by **30/3/70** re-measured at its own commit. ✅ **`.7.4.4` (REPAIR-0254)**: `DELETE /v1/snapshots/{id}` withdraws the caller's citation; tombstoning a shared row is an audited site act. ✅ **`.7.3.2.1` (REPAIR-0253)**: 1.9 GB retained, 99.93 % reproducible payload, 34 fixtures reduced and **0 deleted**. ✅ `.7.2.7`–`.7.2.10` (0249–0252); `.7.2`'s mechanism table is fully discharged. | repo-local |
