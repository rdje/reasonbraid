...
`GET /v1/nodes/inbox` and by the MCP `list_inbox` tool. It is derived from the
columns the channel already writes — never stored in parallel — so it cannot
...
acknowledgement will skip a re-offer it should make. This field published
`acknowledged` for that fact until `migrations/0075`; the
