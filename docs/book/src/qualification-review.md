...
| MCP and A2A | MCP reads now run the HTTP handlers' own authorization, because they call it (`.6.1.1`); the write seam's tenant binding, the untyped body index, listen dedup and the registry's scope remain. A2A qualification currently demonstrates serialization rather than an independent transport peer. | `.6.1`–`.6.3` |
