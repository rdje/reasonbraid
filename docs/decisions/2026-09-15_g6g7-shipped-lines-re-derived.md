...
| (4) | SSRF / DNS-rebinding / redirect / archive-bomb suite | **narrowed** | R0 and R1 have OPPOSITE redirect designs. `fetcher.rs` sets `Policy::none()` and follows redirects manually, re-classifying every hop. `git.rs` sets `Policy::limited(5)` — auto-follow. Its pre-flight `classify()` covers the FIRST destination including an IP literal; behind the hops there is only the DNS hook, and hyper-util skips that for an IP-literal host, in its own words: *"If the host is already an IP addr (v4 or v6), skip resolving the dns and start connecting right away."* So a REDIRECT HOP to a bare address is the one destination nothing classifies, while the module's doc claims "every dial (redirect hops included) passes the destination policy". ⛔ **Corrected the same day**: this row first called the DNS hook R1's "only destination control", which overstated the gap by ignoring the pre-flight. The line stands for R0 and must be re-earned for R1. Source-measured; runtime reproduction is `SIGNOFF-REPAIR.7.2`'s, record `R-44-45-1` |
