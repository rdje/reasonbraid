...
⛔ **bedrock is a GITHUB TEMPLATE REPOSITORY, not a syncable remote** (director, 2026-09-18). A project is instantiated through GitHub's *Use this template* — the whole repository is copied — then cloned and bootstrapped locally, which is exactly this repository's own first two commits (`b932c05 "Initial commit"`, `823c2bc "bootstrapped from bedrock"`), and **reasonbraid's remote set contains no bedrock entry**. ⚠️ bedrock has its own `origin` (`rdje/bedrock.git`) so the template itself can be modified; what does not exist is a link FROM an instantiated project BACK to the template — no pull, no fork relationship, no sync path: `update_scaffold.sh` here names *reasonbraid* as the template source for reasonbraid's OWN descendants. Anything that reaches bedrock is carried by hand.
