...
- [Authority writer coverage, re-derived](docs/tasks/artifacts/signoff_review/authority-writer-coverage.md) — `.3.3.4.3.4`: the `.3.3.4.1` census promoted from a table to a tracked instrument that reproduces its baseline exactly, then re-run. 42 → 39 call sites while the corpus grew; a module split (net zero) separated from a bridge that genuinely went away. Zero obsolete bridges remain; both unguarded families still match their recorded owners.

- [The R2 acquisition join](docs/tasks/artifacts/signoff_review/r2-acquisition-join.md) — `.7.3.3.4.1`/`.7.3.3.4.2`: the success path driven live through the real handler to its snapshot and derivations, asserted against the served bytes, and the mismatch refusal proved to persist nothing. Three deployments isolate the scheme and destination gates; five reverted injections prove both controls go red, including one that keeps the refusal's name and still fails the count. Surfaced the advertised-format finding, censused and repaired under `.7.3.3.5`: the acquisition leg now admits the RANKED pack's own advertised media types, with the bound proved by a live negative control.
- [First complete pre-push checkpoint](docs/tasks/artifacts/signoff_review/checkpoint-7233122.md) — `.11.4.3.1.2`: all eight commands return 0 at source `7233122`, with 40 of 40 PostgreSQL suites, 291 tests, the demonstration's acceptance checks, 16 real browser controls and both scanner gates. Re-derived from receipts and falsified for skips. Remote CI still unrun.
- [Verification strategy proposal](docs/tasks/artifacts/signoff_review/verification-strategy-proposal.md) — capture owner `SIGNOFF-REPAIR.11.5`: director question on exercising a networked agent platform, answered from the six defects repaired on 2026-09-11, none of which was a wrong return value. Four ranked proposals and an explicit MCP assessment. Proposal only; no pivot.
- [Recreated schema privileges](docs/tasks/artifacts/signoff_review/site-operator-schema-usage.md) — `.11.4.3.1.2.14`: the source-165cb3a checkpoint stop root-caused to a fixture recreating `public` without its PUBLIC USAGE grant, and the misclassified refusal it exposed. Reproduced in two suites, falsified control, affected family of six suites green. The full checkpoint has not passed.
- [Exclusively owned extraction inputs](docs/tasks/artifacts/signoff_review/extraction-owned-input.md) — `.7.3.3.3.1`/`.7.3.3.3.2`: runtime-rooted private store, skipped occupied candidates, identity-checked release gated on a finished reader, and R2 responses refused before persistence when they name other bytes. Eighteen controls, the live profiles suite, adjacent suites and strict lint pass. The uncovered acquisition success path has census-backed owner .7.3.3.4.
...
- [Pinned browser runtime](docs/tasks/artifacts/signoff_review/ci-browser-runtime.md) — `.11.4.3.1.2.5`: exact dependency identities, local/CI binding and native Python shutdown-race repair.
