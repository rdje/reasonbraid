...
- [The R2 acquisition join](docs/tasks/artifacts/signoff_review/r2-acquisition-join.md) — `.7.3.3.4.1`: the success path driven live through the real handler to its snapshot and derivations, asserted against the served bytes. Three deployments isolate the scheme and destination gates; three reverted injections prove the control goes red. Surfaced the advertised-format finding, owned by `.7.3.3.5`.
