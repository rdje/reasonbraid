...
| `R-40-42-5` | 2 | owned | `SIGNOFF-REPAIR.9.3` | goal line "Bind desired digests/refs to publication", verbatim. Live: `deployments::assign` validates `is_sha256_hex(&input.desired_digest)` — the SHAPE — and compares it against no publication projection; `policy.rs:2724` supplies `sha256:` plus sixty-four `a`s and asserts 200 |
...
| `R-75-2` | 1 | owned | `SIGNOFF-REPAIR.9.3` | goal line "Bind desired digests/refs to publication". Live: the policy-correction drift control at `policy.rs:2724` assigns `desired_digest: DIGEST` — `sha256:` plus sixty-four `a`s — with `desired_ref: "abc123"`, and the assignment returns 200 against a publication whose projection is neither |
