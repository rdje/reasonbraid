...
DOCTRINES=(
  ...
  "FRONTIER-STATUS|a frontier row's status column agrees with the leaf it names, and row 1 is not a finished leaf — the two are copies of one fact and drifted three times, once at row 1 itself, which is the leaf a fresh session resumes from|scripts/check_frontier_status.py"
  "LEAF-ID-UNIQUE|no two task-tree leaves share one id — a leaf id IS its address, and the one near-miss this project has had was refused by FRONTIER-STATUS only by accident, because that gate resolves an id to the FIRST heading carrying it|scripts/check_leaf_id_unique.sh"
  "SCAFFOLD-COVERAGE|every check the doctrine registry NAMES is carried by the scaffold's NEUTRAL list — the list is a second copy of the registry and nothing derived it, so it drifted by seven checks and a project that pulled the spine could not make its first commit|scripts/check_scaffold_coverage.sh"
