...
# transaction, .2.2 outbox worker, .3.2 node channel, .5.1 authority) against an
# EPHEMERAL server:
...
cargo test -p reasonbraid-server --test atomic_transaction --test outbox_worker --test node_channel --test authority -- --nocapture
